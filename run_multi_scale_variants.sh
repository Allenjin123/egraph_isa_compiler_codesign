...
DEFAULT_BAN_OPS=""             # Banned operators (space-separated, empty = no ban)
...
usage() {
    ...
    echo "  -o, --output-dir DIR       输出根目录 (默认: output/)，所有子目录(ilp/frontend/backend等)均在此下创建"
    ...
    echo "  --ban-ops OPS              空格分隔的禁止指令列表 (例: --ban-ops 'Mul Div')"
...
# Recompute derived paths in case OUTPUT_BASE was overridden by -o
BACKEND_OUTPUT="$OUTPUT_BASE/backend"
...
echo -e "禁止指令: ${GREEN}$([ -n "$BAN_OPS" ] && echo "$BAN_OPS" || echo "无")${NC}"
...
python3 "$EXTRACTOR_DIR/src/ILP/ilp_solver.py" "$PROGRAM_NAME" \\
    ...
    --timeout "$TIMEOUT" \\
    --cost-mode latency \\
    --output "\$SCALE_OUTPUT" ${BAN_OPS:+--ban-ops $BAN_OPS} 2>&1 | tee "$ILP_INTERMEDIATE/ilp_scale_${SCALE}.log"
...
EOF
    ...
    for i in "${!SCALE_ARRAY[@]}"; do
        ...
        if [ -d "$SOL_DIR" ]; then
            # Check if this scale timed out via result.json solve time
            RESULT_JSON="$OUTPUT_BASE/ilp/${PROGRAM_NAME}_scale_${SCALE}/${PROGRAM_NAME}/result/result.json"
            if [ -f "$RESULT_JSON" ]; then
                TIMED_OUT=$(python3 -c "
import json
d = json.load(open('$RESULT_JSON'))
solve = d.get('times', {}).get('solve', 0)
print('yes' if solve >= $TIMEOUT else 'no')
" 2>/dev/null)
                if [ "$TIMED_OUT" = "yes" ]; then
                    echo -e "${YELLOW}  跳过: scale $SCALE (ILP 超时，解质量不可靠)${NC}"
                    continue
                fi
            fi
...
if [ $ACTUAL_VARIANTS -eq 0 ]; then
    echo -e "${YELLOW}警告: 程序 ${PROGRAM_NAME} 未找到任何解文件，跳过此程序${NC}"
    exec 1>&3 2>&4
    exec 3>&- 4>&-
    return 0
else
    ...
    echo "运行重建脚本..."
    if ! OUTPUT_BASE="$OUTPUT_BASE" bash "$FRONTEND_DIR/run_reconstruct.sh" "$PROGRAM_NAME" "$ACTUAL_VARIANTS"; then
...
FRONTEND_OUTPUT="$OUTPUT_BASE/frontend/$PROGRAM_NAME"
...
if [ ! -f "$FRONTEND_SCRIPT" ]; then
    ...
    if ! (cd "$FRONTEND_DIR" && FRONTEND_OUTPUT_BASE="$OUTPUT_BASE/frontend" bash run_full_analysis.sh "${PROGRAM_NAMES[@]}"); then
...
if [ "$SKIP_FRONTEND" = false ]; then
    if [ -f "$COMPLETE_ANALYSIS_SCRIPT" ]; then
        ...
        if (cd "$BENCHMARK_DIR" && bash run_complete_analysis.sh "${PROGRAM_NAMES[@]}"); then
