...
Usage:
    python parse_results_power_analytical.py [output_base]
...
def compute_summary(program_name: str, original: Dict, candidates: List[Dict],
    ...
    def evaluate_at_freq(e, run_freq):
        """Compute power/energy at a given frequency."""
        freq_norm = run_freq / f_ref
    ...
    def find_best(constraint_factor, objective):
        ...
        min_area: iso-frequency (f_ref). Latency constraint uses cycle_norm
                  directly (no frequency scaling).
        min_energy: max frequency. Latency constraint uses latency_norm
                    (= cycle_norm / freq_norm).
        """
        results = []
        for e in enriched:
            if objective == 'min_area':
                # Latency constraint on cycles only
                if not math.isinf(constraint_factor):
                    if e['cycle_norm'] > constraint_factor:
                        continue
                results.append(evaluate_at_freq(e, f_ref))
            else:  # min_energy
                # Latency constraint on wall-clock time
                freq_norm = e['variant_freq'] / f_ref
                latency_norm = e['cycle_norm'] / freq_norm
                if not math.isinf(constraint_factor):
                    if latency_norm > constraint_factor:
                        continue
                results.append(evaluate_at_freq(e, e['variant_freq']))
...
def _generate_latex_table_core(summaries_m1: List[Dict], summaries_m2: List[Dict],
    ...
    lines = []
    lines.append('% Auto-generated by parse_results_power_analytical.py — do not edit manually')
...
def main():
    ...
    backend_dir_m1 = project_dir / "output_m1_final" / "backend"
    ...
    print("="*80)
    print(f"Backend directory (M1): {backend_dir_m1}")
    print(f"Backend directory (M2): {backend_dir_m2}")
    ...
    # --- Mode 1: 99% dynamic, 1% static (from output_m1_final) ---
    print("Parsing M1 results from output_m1_final...")
    json_files_m1 = find_all_result_jsons(backend_dir_m1)
    print(f"  Found {len(json_files_m1)} analysis result files")

    parsed_m1 = []
    for json_path in json_files_m1:
        program_name = json_path.parent.name
        print(f"  Parsing {program_name}...", end=" ")
        result = parse_all_variants(json_path)
        if result:
            parsed_m1.append(result)
            print("OK")
        else:
            print("SKIP")

    print(f"  Total programs: {len(parsed_m1)}")
    print()
    ...
    # --- Mode 2: 1% dynamic, 99% static (from output_m2) ---
    print("Parsing M2 results from output_m2...")
    json_files_m2 = find_all_result_jsons(backend_dir_m2)
    print(f"  Found {len(json_files_m2)} analysis result files")

    parsed_m2 = []
    for json_path in json_files_m2:
        program_name = json_path.parent.name
        print(f"  Parsing {program_name}...", end=" ")
        result = parse_all_variants(json_path)
        if result:
            parsed_m2.append(result)
            print("OK")
        else:
            print("SKIP")

    print(f"  Total programs: {len(parsed_m2)}")
    print()
    ...
    generate_latex_table_minA(summaries_m1, summaries_m2,
                              str(script_dir / 'table_minA_analytical.tex'))
    generate_latex_table_minE(summaries_m1, summaries_m2,
                              str(script_dir / 'table_minE_analytical.tex'))
