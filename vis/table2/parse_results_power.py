...
def load_pnr_summaries(pnr_csv_path: Path, sim_cycles: Dict) -> List[Dict]:
    """Load real PNR data and build summary dicts compatible with table generators.

    The PNR CSV has absolute area/power/freq for baseline and each
    (objective, constraint) combination.  We normalize to baseline and
    compute energy using RTL sim cycles.

    Returns list of summary dicts (one per benchmark), same format as
    compute_summary() output.
    """
    if not pnr_csv_path.exists():
        print(f"Warning: PNR CSV not found at {pnr_csv_path}")
        return []
    ...
    with open(pnr_csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # RTL cycles for original
            orig_cycles = sim_cycles.get((app_name, 'original'))

            summary = {'application_name': app_name}

            # For each (objective, constraint) in the CSV
            for prefix, obj_tag in [('minA', 'minA'), ('minE', 'minE')]:
                for cf_label, csv_tag in [('1.2x', '1.2x'),
                    ...
                    power_norm = v_power / baseline_power
                    freq_norm = v_freq / baseline_freq

                    # Cycle norm from RTL simulation
                    ...
                    # Iso-freq power: scale PNR power back to baseline freq
                    # P_pnr is at v_freq; at baseline freq, dynamic scales
                    # linearly with freq.  Approximate: P_iso ≈ P_pnr / freq_norm
                    # (assuming mostly dynamic).  Use raw power_norm as
                    # conservative iso-freq estimate since PNR already captures
                    # the real design.
                    power_iso_norm = power_norm
                    ...
                    summary[f'{key_prefix}_freq'] = v_freq
    ...
    print(f"  Loaded {len(summaries)} benchmarks from PNR data")
    return summaries
...
def main():
    ...
    pnr_csv_path = script_dir / "AESIP result - ALL_VAR_PNR.csv"
    ...
    print("="*80)
    print("Parsing Analysis Results (Real PNR + Analytical Model)")
    print("="*80)
    print(f"Backend directory: {backend_dir}")
    print(f"Sim results: {sim_csv_path} ({len(sim_cycles)} entries)")
    print(f"PNR data (M1): {pnr_csv_path}")
    print(f"Output: {output_m1} (Mode 1: real PNR)")
    print(f"        {output_m2} (Mode 2: analytical)")
    print(f"Baseline: original program (= 1.0)")
    print(f"Cycle counts: RTL simulation (sim_results.csv)")
    print(f"M2 power model: P(f) = dyn_frac * P_dyn_base * f/f_ref + static_frac * area_norm")
    print(f"Energy model: E = power_norm * cycle_norm / freq_norm")
    print(f"Latency constraints: {constraint_strs}")
    print("="*80)
    print()

    # --- Mode 1: Real PNR data ---
    print("Loading real PNR data for M1...")
    summaries_m1 = load_pnr_summaries(pnr_csv_path, sim_cycles)
    generate_csv(summaries_m1, output_m1)

    # --- Mode 2: Analytical model (1% dynamic, 99% static) ---
    print()
    print("Computing analytical model for M2...")
