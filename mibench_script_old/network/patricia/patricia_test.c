...
#include <string.h>
#include <assert.h>
/* #include <err.h> */  /* Not available in RISC-V embedded toolchain */
#include <errno.h>
#include <fcntl.h>
#include <unistd.h>

/* Network headers not available in RISC-V embedded toolchain */
...
/* Manual definitions for embedded environment */
struct in_addr {
	unsigned int s_addr;
};

/* Byte order conversion - for embedded we assume same endianness */
#define htonl(x) (x)
#define ntohl(x) (x)
