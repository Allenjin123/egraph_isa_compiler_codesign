...
BIN_PATH="./dijkstra_large$BIN_SUFFIX"
...
if [ -x "$RISCV/bin/riscv32-unknown-elf-objdump" ]; then
    "$RISCV/bin/riscv32-unknown-elf-objdump" -d -M no-aliases "$BIN_PATH" > "$BENCH_DIR/dijkstra_large$BIN_SUFFIX.dump"
fi
...
$MIBENCH_RUN "$BIN_PATH" input.dat > output_large.txt
