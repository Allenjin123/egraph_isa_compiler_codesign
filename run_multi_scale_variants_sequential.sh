#!/bin/bash
# ============================================================================
# Multi-Scale Variant Generation Script (Sequential Synthesis, No GP/Original)
#
...
# Synthesis runs sequentially. Records saturation, extraction, and synthesis time.
#
# Usage: ./run_multi_scale_variants_sequential.sh <program_name> [options]
# ============================================================================

set -e  # Exit on error

# Default configuration
DEFAULT_SCALES="0 0.05 0.5 1"
DEFAULT_K=1
DEFAULT_TIMEOUT=180
DEFAULT_PROGRAM_PARALLEL=1     # Number of programs to process in parallel
DEFAULT_ILP_PARALLEL=24        # Number of ILP scaling factors to run in parallel per program
DEFAULT_SYNTH_PARALLEL=1       # Sequential synthesis (one at a time)
...
echo -e "合成并行数: ${GREEN}${SYNTH_PARALLEL}${NC} (顺序合成)"
...
if [ "$RUN_SATURATION" = true ]; then
    ...
    SATURATION_END=$(date +%s)
    SATURATION_TIME=$((SATURATION_END - SATURATION_START))

    if [ $PROGRAM_FAILED -eq 0 ]; then
        echo -e "${GREEN}  ✓ 饱和完成 (耗时: ${SATURATION_TIME}秒)${NC}"
    fi
    echo ""
fi
...
EOF
    ...
    echo -e "${GREEN}  提取总耗时: ${EXTRACTION_TIME}秒${NC}"
    echo ""
fi
...
echo -e "${BLUE}步骤 6/6: 顺序分析所有变体的面积和延迟 (${SYNTH_PARALLEL} 个进程)...${NC}"

# Swap in timed synthesis script to collect per-step timing (scorr, techmap, etc.)
SYNTH_SCRIPT_DIR="$SCRIPT_DIR/PdatScorrWrapper/ScorrPdat"
SYNTH_SCRIPT_ORIG="$SYNTH_SCRIPT_DIR/synth_core_simplified.sh"
SYNTH_SCRIPT_TIMED="$SYNTH_SCRIPT_DIR/synth_core_simplified_timed.sh"
SYNTH_SCRIPT_BACKUP="$SYNTH_SCRIPT_DIR/synth_core_simplified.sh.bak"

if [ -f "$SYNTH_SCRIPT_TIMED" ]; then
    cp "$SYNTH_SCRIPT_ORIG" "$SYNTH_SCRIPT_BACKUP"
    cp "$SYNTH_SCRIPT_TIMED" "$SYNTH_SCRIPT_ORIG"
    echo -e "${CYAN}  已启用计时合成脚本 (synth_core_simplified_timed.sh)${NC}"
fi
...
# Restore original synthesis script
if [ -f "$SYNTH_SCRIPT_BACKUP" ]; then
    mv "$SYNTH_SCRIPT_BACKUP" "$SYNTH_SCRIPT_ORIG"
    echo -e "${CYAN}  已恢复原始合成脚本${NC}"
fi

SYNTHESIS_END=$(date +%s)
SYNTHESIS_TIME=$((SYNTHESIS_END - SYNTHESIS_START))

# Collect per-variant scorr/techmap timing from synthesis output directories
SYNTH_TIMING_OUTPUT="$SCRIPT_DIR/PdatScorrWrapper/ScorrPdat/output"
VARIANT_TIMING_CSV="${OUTPUT_DIR}/variant_synth_timing.csv"
echo "variant,synlig_techmap_s,abc_scorr_s,odc_scorr_s,gate_synth_s,total_s" > "$VARIANT_TIMING_CSV"
TOTAL_SYNLIG=0; TOTAL_ABC_SCORR=0; TOTAL_ODC_SCORR=0; TOTAL_GATE=0

for timing_file in "$SYNTH_TIMING_OUTPUT"/${PROGRAM_NAME}_variant_*/synth_timing.csv; do
    if [ -f "$timing_file" ]; then
        # Read data lines (skip header)
        while IFS=',' read -r dsl synlig abc_scorr odc_scorr gate total; do
            [ "$dsl" = "dsl" ] && continue  # skip header
            echo "${dsl},${synlig},${abc_scorr},${odc_scorr},${gate},${total}" >> "$VARIANT_TIMING_CSV"
            TOTAL_SYNLIG=$((TOTAL_SYNLIG + synlig))
            TOTAL_ABC_SCORR=$((TOTAL_ABC_SCORR + abc_scorr))
            TOTAL_ODC_SCORR=$((TOTAL_ODC_SCORR + odc_scorr))
            TOTAL_GATE=$((TOTAL_GATE + gate))
        done < "$timing_file"
    fi
done

echo -e "${CYAN}--- 合成子步骤耗时汇总 ---${NC}"
echo -e "  synlig/techmap 总计: ${GREEN}${TOTAL_SYNLIG}秒${NC}"
echo -e "  ABC/scorr 总计:      ${GREEN}${TOTAL_ABC_SCORR}秒${NC}"
echo -e "  ODC ABC/scorr 总计:  ${GREEN}${TOTAL_ODC_SCORR}秒${NC}"
echo -e "  Gate synthesis 总计: ${GREEN}${TOTAL_GATE}秒${NC}"
echo -e "  详细数据: ${GREEN}${VARIANT_TIMING_CSV}${NC}"

if [ $SYNTH_EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}✓ Pareto 分析完成 (合成耗时: ${SYNTHESIS_TIME}秒)${NC}"
    ANALYSIS_SUCCESS=1
else
    echo -e "${YELLOW}⚠ 分析失败（可能缺少合成工具）(合成耗时: ${SYNTHESIS_TIME}秒)${NC}"
    ANALYSIS_SUCCESS=0
fi
...
echo ""
echo -e "${CYAN}--- 各阶段耗时 ---${NC}"
echo -e "饱和时间:   ${GREEN}${SATURATION_TIME}秒${NC}"
echo -e "提取时间:   ${GREEN}${EXTRACTION_TIME}秒${NC}"
echo -e "合成时间:   ${GREEN}${SYNTHESIS_TIME}秒${NC}"
TOTAL_PROGRAM_TIME=$((SATURATION_TIME + EXTRACTION_TIME + SYNTHESIS_TIME))
echo -e "总计:       ${GREEN}${TOTAL_PROGRAM_TIME}秒${NC}"
echo ""
...
Timing:
- Saturation time: ${SATURATION_TIME}s
- Extraction time: ${EXTRACTION_TIME}s
- Synthesis time: ${SYNTHESIS_TIME}s
- Total: $((SATURATION_TIME + EXTRACTION_TIME + SYNTHESIS_TIME))s
...
# Write per-program timing to CSV
TIMING_CSV="${OUTPUT_BASE}/timing_results.csv"
echo "${PROGRAM_NAME},${SATURATION_TIME},${EXTRACTION_TIME},${SYNTHESIS_TIME},${TOTAL_SYNLIG},${TOTAL_ABC_SCORR},${TOTAL_ODC_SCORR},${TOTAL_GATE},$((SATURATION_TIME + EXTRACTION_TIME + SYNTHESIS_TIME))" >> "$TIMING_CSV"
echo -e "${GREEN}  计时数据已追加至: $TIMING_CSV${NC}"
...
# Initialize timing CSV (fresh for each run)
TIMING_CSV="${OUTPUT_BASE}/timing_results.csv"
echo "program,saturation_s,extraction_s,synthesis_s,synlig_techmap_s,abc_scorr_s,odc_scorr_s,gate_synth_s,total_s" > "$TIMING_CSV"
echo -e "${CYAN}计时结果将保存至: $TIMING_CSV${NC}"
