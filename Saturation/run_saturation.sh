...
# 使用方法: ./run_saturation.sh [--steps 1,2,3] [program_name1] [program_name2] [...]
# 示例: ./run_saturation.sh                          # 遍历所有程序
#       ./run_saturation.sh dijkstra_small_O3
#       ./run_saturation.sh --steps 1,2 dijkstra_small_O3
#       ./run_saturation.sh --steps 3                # 对所有程序只跑步骤3
...
if [ $# -eq 0 ]; then
    ...
    if [ ! -d "$FRONTEND_DIR" ] || [ -z "$(ls -A "$FRONTEND_DIR" 2>/dev/null)" ]; then
        echo -e "${RED}错误: 找不到任何程序，目录不存在或为空: $FRONTEND_DIR${NC}"
        exit 1
    fi
...
process_program() {
    ...
    if should_run_step 1; then
        ...
        echo -e "${YELLOW}跳过步骤 1: 生成 .egg 文件${NC}"
        echo ""
    fi

    # ============================================================
    # 步骤 2/3: 提取 eclass 信息 (extract_eclasses.py)
    # ============================================================
    if should_run_step 2; then
        echo -e "${MAGENTA}步骤 2/3: 提取 E-class 注释${NC}"
        ...
        if ! python3 "$SCRIPT_DIR/extract_eclasses.py" "$PROGRAM_DIR" -v --egglog "$EGGLOG_PATH"; then
            echo -e "${RED}步骤 2 失败: extract_eclasses.py${NC}"
            FAIL_COUNT=$((FAIL_COUNT + 1))
        ...
        echo -e "${YELLOW}跳过步骤 2: 提取 E-class 注释${NC}"
        echo ""
    fi

    # ============================================================
    # 步骤 3/3: 运行 egglog 生成可视化 (run_egglog_all.py)
    # ============================================================
    if should_run_step 3; then
        echo -e "${MAGENTA}步骤 3/3: 运行 E-graph 饱和分析${NC}"
        echo "输入: $PROGRAM_DIR/basic_blocks_egglog/*.egg"
        echo "输出: $PROGRAM_DIR/basic_blocks_json/"

        if ! python3 "$SCRIPT_DIR/run_egglog_all.py" "$PROGRAM_DIR" -v; then
            echo -e "${RED}步骤 3 失败: run_egglog_all.py${NC}"
            FAIL_COUNT=$((FAIL_COUNT + 1))
        ...
        echo -e "${YELLOW}跳过步骤 3: 运行 E-graph 饱和分析${NC}"
        echo ""
    fi
