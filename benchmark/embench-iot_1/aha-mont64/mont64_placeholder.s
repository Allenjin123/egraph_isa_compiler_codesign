	.file	"mont64.c"
	.option nopic
	.option norelax
	.attribute arch, "rv32i2p1_m2p0_zicsr2p0_zifencei2p0_zmmul1p0"
	.attribute unaligned_access, 0
	.attribute stack_align, 16
	.text
	.align	2
	.globl	initialise_board
	.type	initialise_board, @function
initialise_board:
	jalr	zero,ra,0
	.size	initialise_board, .-initialise_board
	.align	2
	.globl	start_trigger
	.type	start_trigger, @function
start_trigger:
	jalr	zero,ra,0
	.size	start_trigger, .-start_trigger
	.align	2
	.globl	stop_trigger
	.type	stop_trigger, @function
stop_trigger:
	jalr	zero,ra,0
	.size	stop_trigger, .-stop_trigger
	.align	2
	.globl	mulul64
	.type	mulul64, @function
mulul64:
	callmul	a6,a1,a2
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a0,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a0,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a2,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a2,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a0,op_25
	addi	op_29,x0,16
	srl	op_28,a0,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a2,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a2,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a0,op_46
	addi	op_50,x0,16
	srl	op_49,a0,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a2,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a2,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a0,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a0,op_70
	addi	op_78,x0,16
	srl	op_77,a2,op_78
	addi	op_82,x0,16
	srl	op_81,a2,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a0,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a0,op_91
	addi	op_99,x0,16
	srl	op_98,a2,op_99
	addi	op_103,x0,16
	srl	op_102,a2,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a0,op_119
	addi	op_123,x0,16
	srl	op_122,a0,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a2,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a2,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a0,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a0,op_141
	addi	op_149,x0,16
	srl	op_148,a2,op_149
	addi	op_153,x0,16
	srl	op_152,a2,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a0,op_163
	addi	op_167,x0,16
	srl	op_166,a0,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a2,op_174
	addi	op_178,x0,16
	srl	op_177,a2,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t1,op_0,op_113
	callmul	a7,a0,a3
	sub	op_0,x0,a6
	sub	t1,t1,op_0
	bgeu	t1,a6,.+8
	jal	x0,.+12
	addi	a6,x0,0
	jal	x0,.+8
	addi	a6,x0,1
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a1,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a1,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a2,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a2,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a1,op_25
	addi	op_29,x0,16
	srl	op_28,a1,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a2,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a2,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a1,op_46
	addi	op_50,x0,16
	srl	op_49,a1,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a2,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a2,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a1,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a1,op_70
	addi	op_78,x0,16
	srl	op_77,a2,op_78
	addi	op_82,x0,16
	srl	op_81,a2,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a1,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a1,op_91
	addi	op_99,x0,16
	srl	op_98,a2,op_99
	addi	op_103,x0,16
	srl	op_102,a2,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a1,op_119
	addi	op_123,x0,16
	srl	op_122,a1,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a2,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a2,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a1,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a1,op_141
	addi	op_149,x0,16
	srl	op_148,a2,op_149
	addi	op_153,x0,16
	srl	op_152,a2,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a1,op_163
	addi	op_167,x0,16
	srl	op_166,a1,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a2,op_174
	addi	op_178,x0,16
	srl	op_177,a2,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t4,op_0,op_113
	sub	op_0,x0,a7
	sub	t1,t1,op_0
	sw	t1,4(a5)
	bgeu	t1,a7,.+8
	jal	x0,.+12
	addi	a7,x0,0
	jal	x0,.+8
	addi	a7,x0,1
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a0,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a0,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a3,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a3,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a0,op_25
	addi	op_29,x0,16
	srl	op_28,a0,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a3,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a3,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a0,op_46
	addi	op_50,x0,16
	srl	op_49,a0,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a3,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a3,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a0,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a0,op_70
	addi	op_78,x0,16
	srl	op_77,a3,op_78
	addi	op_82,x0,16
	srl	op_81,a3,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a0,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a0,op_91
	addi	op_99,x0,16
	srl	op_98,a3,op_99
	addi	op_103,x0,16
	srl	op_102,a3,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a0,op_119
	addi	op_123,x0,16
	srl	op_122,a0,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a3,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a3,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a0,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a0,op_141
	addi	op_149,x0,16
	srl	op_148,a3,op_149
	addi	op_153,x0,16
	srl	op_152,a3,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a0,op_163
	addi	op_167,x0,16
	srl	op_166,a0,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a3,op_174
	addi	op_178,x0,16
	srl	op_177,a3,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t3,op_0,op_113
	sub	op_0,x0,a6
	sub	a6,t4,op_0
	callmul	t1,a1,a3
	sub	op_0,x0,a7
	sub	a7,t3,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a1,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a1,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a3,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a3,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a1,op_25
	addi	op_29,x0,16
	srl	op_28,a1,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a3,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a3,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a1,op_46
	addi	op_50,x0,16
	srl	op_49,a1,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a3,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a3,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a1,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a1,op_70
	addi	op_78,x0,16
	srl	op_77,a3,op_78
	addi	op_82,x0,16
	srl	op_81,a3,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a1,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a1,op_91
	addi	op_99,x0,16
	srl	op_98,a3,op_99
	addi	op_103,x0,16
	srl	op_102,a3,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a1,op_119
	addi	op_123,x0,16
	srl	op_122,a1,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a3,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a3,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a1,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a1,op_141
	addi	op_149,x0,16
	srl	op_148,a3,op_149
	addi	op_153,x0,16
	srl	op_152,a3,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a1,op_163
	addi	op_167,x0,16
	srl	op_166,a1,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a3,op_174
	addi	op_178,x0,16
	srl	op_177,a3,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a1,op_0,op_113
	sub	op_0,x0,t1
	sub	a6,a6,op_0
	sub	op_0,x0,a6
	sub	a7,a7,op_0
	bgeu	a6,t1,.+8
	jal	x0,.+12
	addi	t1,x0,0
	jal	x0,.+8
	addi	t1,x0,1
	bgeu	a7,a6,.+8
	jal	x0,.+12
	addi	a6,x0,0
	jal	x0,.+8
	addi	a6,x0,1
	callmul	a0,a0,a2
	sub	op_0,x0,t1
	sub	t1,a1,op_0
	sub	op_0,x0,a6
	sub	a6,t1,op_0
	sw	a0,0(a5)
	sw	a6,4(a4)
	sw	a7,0(a4)
	jalr	zero,ra,0
	.size	mulul64, .-mulul64
	.align	2
	.globl	modul64
	.type	modul64, @function
modul64:
	addi	sp,sp,-16
	sw	s0,12(sp)
	addi	t3,zero,0
	addi	s0,a4,0
	addi	t0,zero,64
.L9:
	addi	op_0,x0,31
	srl	a7,a0,op_0
	addi	op_0,x0,1
	sll	a6,a1,op_0
	sub	op_0,x0,a7
	sub	a6,a6,op_0
	addi	op_0,x0,31
	sra	t1,a1,op_0
	addi	op_0,x0,1
	sll	a7,a3,op_0
	addi	op_0,x0,31
	srl	a1,a3,op_0
	addi	op_0,x0,1
	sll	a0,a0,op_0
	addi	op_0,x0,31
	srl	a3,a2,op_0
	or	a0,a1,a0
	sub	op_0,x0,a3
	sub	a7,a7,op_0
	or	t4,t1,a6
	addi	a1,a6,0
	addi	t3,t3,1
	addi	op_0,x0,1
	sll	a2,a2,op_0
	or	t1,t1,a0
	addi	a3,a7,0
	bgeu	t4,a5,.+8
	jal	x0,.L7
	sub	t6,a0,s0
	addi	t5,a2,1
	bgeu	a0,t6,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	bgeu	t5,a2,.+8
	jal	x0,.+12
	addi	t2,x0,0
	jal	x0,.+8
	addi	t2,x0,1
	sub	a6,a6,a5
	bne	a5,t4,.L10
	bgeu	t1,s0,.+8
	jal	x0,.L7
.L10:
	addi	a0,t6,0
	sub	a1,a6,a4
	addi	a2,t5,0
	sub	op_0,x0,t2
	sub	a3,a7,op_0
.L7:
	bne	t3,t0,.L9
	lw	s0,12(sp)
	addi	sp,sp,16
	jalr	zero,ra,0
	.size	modul64, .-modul64
	.align	2
	.globl	montmul
	.type	montmul, @function
montmul:
	callmul	t1,a1,a2
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a0,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a0,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a2,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a2,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a0,op_25
	addi	op_29,x0,16
	srl	op_28,a0,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a2,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a2,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a0,op_46
	addi	op_50,x0,16
	srl	op_49,a0,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a2,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a2,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a0,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a0,op_70
	addi	op_78,x0,16
	srl	op_77,a2,op_78
	addi	op_82,x0,16
	srl	op_81,a2,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a0,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a0,op_91
	addi	op_99,x0,16
	srl	op_98,a2,op_99
	addi	op_103,x0,16
	srl	op_102,a2,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a0,op_119
	addi	op_123,x0,16
	srl	op_122,a0,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a2,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a2,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a0,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a0,op_141
	addi	op_149,x0,16
	srl	op_148,a2,op_149
	addi	op_153,x0,16
	srl	op_152,a2,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a0,op_163
	addi	op_167,x0,16
	srl	op_166,a0,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a2,op_174
	addi	op_178,x0,16
	srl	op_177,a2,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t3,op_0,op_113
	callmul	t4,a0,a3
	sub	op_0,x0,t1
	sub	t3,t3,op_0
	bgeu	t3,t1,.+8
	jal	x0,.+12
	addi	t1,x0,0
	jal	x0,.+8
	addi	t1,x0,1
	callmul	t5,a0,a2
	sub	op_0,x0,t4
	sub	t3,t3,op_0
	bgeu	t3,t4,.+8
	jal	x0,.+12
	addi	t6,x0,0
	jal	x0,.+8
	addi	t6,x0,1
	callmul	a7,a7,t5
	callmul	t4,t3,a6
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a1,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a1,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a2,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a2,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a1,op_25
	addi	op_29,x0,16
	srl	op_28,a1,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a2,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a2,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a1,op_46
	addi	op_50,x0,16
	srl	op_49,a1,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a2,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a2,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a1,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a1,op_70
	addi	op_78,x0,16
	srl	op_77,a2,op_78
	addi	op_82,x0,16
	srl	op_81,a2,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a1,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a1,op_91
	addi	op_99,x0,16
	srl	op_98,a2,op_99
	addi	op_103,x0,16
	srl	op_102,a2,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a1,op_119
	addi	op_123,x0,16
	srl	op_122,a1,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a2,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a2,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a1,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a1,op_141
	addi	op_149,x0,16
	srl	op_148,a2,op_149
	addi	op_153,x0,16
	srl	op_152,a2,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a1,op_163
	addi	op_167,x0,16
	srl	op_166,a1,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a2,op_174
	addi	op_178,x0,16
	srl	op_177,a2,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a2,op_0,op_113
	sub	op_0,x0,a7
	sub	a7,t4,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a6,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a6,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,t5,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,t5,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a6,op_25
	addi	op_29,x0,16
	srl	op_28,a6,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,t5,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,t5,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a6,op_46
	addi	op_50,x0,16
	srl	op_49,a6,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,t5,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,t5,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a6,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a6,op_70
	addi	op_78,x0,16
	srl	op_77,t5,op_78
	addi	op_82,x0,16
	srl	op_81,t5,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a6,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a6,op_91
	addi	op_99,x0,16
	srl	op_98,t5,op_99
	addi	op_103,x0,16
	srl	op_102,t5,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a6,op_119
	addi	op_123,x0,16
	srl	op_122,a6,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,t5,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,t5,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a6,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a6,op_141
	addi	op_149,x0,16
	srl	op_148,t5,op_149
	addi	op_153,x0,16
	srl	op_152,t5,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a6,op_163
	addi	op_167,x0,16
	srl	op_166,a6,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,t5,op_174
	addi	op_178,x0,16
	srl	op_177,t5,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t4,op_0,op_113
	sub	op_0,x0,a7
	sub	a7,t4,op_0
	sub	op_0,x0,t1
	sub	t4,a2,op_0
	callmul	t1,a1,a3
	callmul	a6,a6,t5
	sub	op_0,x0,t4
	sub	t1,t1,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a0,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a0,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a3,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a3,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a0,op_25
	addi	op_29,x0,16
	srl	op_28,a0,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a3,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a3,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a0,op_46
	addi	op_50,x0,16
	srl	op_49,a0,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a3,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a3,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a0,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a0,op_70
	addi	op_78,x0,16
	srl	op_77,a3,op_78
	addi	op_82,x0,16
	srl	op_81,a3,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a0,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a0,op_91
	addi	op_99,x0,16
	srl	op_98,a3,op_99
	addi	op_103,x0,16
	srl	op_102,a3,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a0,op_119
	addi	op_123,x0,16
	srl	op_122,a0,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a3,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a3,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a0,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a0,op_141
	addi	op_149,x0,16
	srl	op_148,a3,op_149
	addi	op_153,x0,16
	srl	op_152,a3,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a0,op_163
	addi	op_167,x0,16
	srl	op_166,a0,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a3,op_174
	addi	op_178,x0,16
	srl	op_177,a3,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a0,op_0,op_113
	callmul	a2,a7,a4
	sub	op_0,x0,t6
	sub	a0,a0,op_0
	bgeu	t1,t4,.+8
	jal	x0,.+12
	addi	t6,x0,0
	jal	x0,.+8
	addi	t6,x0,1
	sub	op_0,x0,t1
	sub	a0,a0,op_0
	bgeu	a0,t1,.+8
	jal	x0,.+12
	addi	t1,x0,0
	jal	x0,.+8
	addi	t1,x0,1
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a6,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a6,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a4,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a4,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a6,op_25
	addi	op_29,x0,16
	srl	op_28,a6,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a4,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a4,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a6,op_46
	addi	op_50,x0,16
	srl	op_49,a6,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a4,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a4,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a6,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a6,op_70
	addi	op_78,x0,16
	srl	op_77,a4,op_78
	addi	op_82,x0,16
	srl	op_81,a4,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a6,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a6,op_91
	addi	op_99,x0,16
	srl	op_98,a4,op_99
	addi	op_103,x0,16
	srl	op_102,a4,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a6,op_119
	addi	op_123,x0,16
	srl	op_122,a6,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a4,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a4,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a6,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a6,op_141
	addi	op_149,x0,16
	srl	op_148,a4,op_149
	addi	op_153,x0,16
	srl	op_152,a4,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a6,op_163
	addi	op_167,x0,16
	srl	op_166,a6,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a4,op_174
	addi	op_178,x0,16
	srl	op_177,a4,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t4,op_0,op_113
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a1,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a1,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a3,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a3,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a1,op_25
	addi	op_29,x0,16
	srl	op_28,a1,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a3,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a3,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a1,op_46
	addi	op_50,x0,16
	srl	op_49,a1,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a3,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a3,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a1,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a1,op_70
	addi	op_78,x0,16
	srl	op_77,a3,op_78
	addi	op_82,x0,16
	srl	op_81,a3,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a1,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a1,op_91
	addi	op_99,x0,16
	srl	op_98,a3,op_99
	addi	op_103,x0,16
	srl	op_102,a3,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a1,op_119
	addi	op_123,x0,16
	srl	op_122,a1,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a3,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a3,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a1,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a1,op_141
	addi	op_149,x0,16
	srl	op_148,a3,op_149
	addi	op_153,x0,16
	srl	op_152,a3,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a1,op_163
	addi	op_167,x0,16
	srl	op_166,a1,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a3,op_174
	addi	op_178,x0,16
	srl	op_177,a3,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a1,op_0,op_113
	sub	op_0,x0,a2
	sub	a3,t4,op_0
	bgeu	a3,a2,.+8
	jal	x0,.+12
	addi	a2,x0,0
	jal	x0,.+8
	addi	a2,x0,1
	callmul	t4,a6,a5
	sub	op_0,x0,t6
	sub	t6,a1,op_0
	sub	op_0,x0,t1
	sub	t1,t6,op_0
	sub	op_0,x0,t4
	sub	t6,a3,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a7,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a7,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a4,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a4,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a7,op_25
	addi	op_29,x0,16
	srl	op_28,a7,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a4,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a4,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a7,op_46
	addi	op_50,x0,16
	srl	op_49,a7,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a4,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a4,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a7,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a7,op_70
	addi	op_78,x0,16
	srl	op_77,a4,op_78
	addi	op_82,x0,16
	srl	op_81,a4,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a7,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a7,op_91
	addi	op_99,x0,16
	srl	op_98,a4,op_99
	addi	op_103,x0,16
	srl	op_102,a4,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a7,op_119
	addi	op_123,x0,16
	srl	op_122,a7,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a4,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a4,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a7,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a7,op_141
	addi	op_149,x0,16
	srl	op_148,a4,op_149
	addi	op_153,x0,16
	srl	op_152,a4,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a7,op_163
	addi	op_167,x0,16
	srl	op_166,a7,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a4,op_174
	addi	op_178,x0,16
	srl	op_177,a4,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a1,op_0,op_113
	bgeu	t6,t4,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	callmul	t4,a7,a5
	sub	op_0,x0,a2
	sub	a2,a1,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a7,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a7,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a5,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a5,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a7,op_25
	addi	op_29,x0,16
	srl	op_28,a7,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a5,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a5,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a7,op_46
	addi	op_50,x0,16
	srl	op_49,a7,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a5,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a5,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a7,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a7,op_70
	addi	op_78,x0,16
	srl	op_77,a5,op_78
	addi	op_82,x0,16
	srl	op_81,a5,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a7,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a7,op_91
	addi	op_99,x0,16
	srl	op_98,a5,op_99
	addi	op_103,x0,16
	srl	op_102,a5,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a7,op_119
	addi	op_123,x0,16
	srl	op_122,a7,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a5,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a5,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a7,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a7,op_141
	addi	op_149,x0,16
	srl	op_148,a5,op_149
	addi	op_153,x0,16
	srl	op_152,a5,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a7,op_163
	addi	op_167,x0,16
	srl	op_166,a7,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a5,op_174
	addi	op_178,x0,16
	srl	op_177,a5,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a7,op_0,op_113
	sub	op_0,x0,t4
	sub	a1,a0,op_0
	sub	op_0,x0,a1
	sub	a2,a2,op_0
	bgeu	a1,t4,.+8
	jal	x0,.+12
	addi	t4,x0,0
	jal	x0,.+8
	addi	t4,x0,1
	bgeu	a2,a1,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a6,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a6,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a5,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a5,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a6,op_25
	addi	op_29,x0,16
	srl	op_28,a6,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a5,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a5,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a6,op_46
	addi	op_50,x0,16
	srl	op_49,a6,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a5,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a5,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a6,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a6,op_70
	addi	op_78,x0,16
	srl	op_77,a5,op_78
	addi	op_82,x0,16
	srl	op_81,a5,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a6,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a6,op_91
	addi	op_99,x0,16
	srl	op_98,a5,op_99
	addi	op_103,x0,16
	srl	op_102,a5,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a6,op_119
	addi	op_123,x0,16
	srl	op_122,a6,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a5,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a5,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a6,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a6,op_141
	addi	op_149,x0,16
	srl	op_148,a5,op_149
	addi	op_153,x0,16
	srl	op_152,a5,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a6,op_163
	addi	op_167,x0,16
	srl	op_166,a6,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a5,op_174
	addi	op_178,x0,16
	srl	op_177,a5,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t0,op_0,op_113
	sub	op_0,x0,a7
	sub	a7,t1,op_0
	sub	op_0,x0,t4
	sub	t4,a7,op_0
	sub	op_0,x0,a1
	sub	a1,t4,op_0
	callmul	a6,a6,a4
	sub	op_0,x0,a3
	sub	a3,t0,op_0
	sub	op_0,x0,a2
	sub	a3,a3,op_0
	bgeu	a3,a2,.+8
	jal	x0,.+12
	addi	a2,x0,0
	jal	x0,.+8
	addi	a2,x0,1
	sub	op_0,x0,a2
	sub	a2,a1,op_0
	addi	a7,a2,0
	addi	a1,a3,0
	sub	op_0,x0,a6
	sub	t4,t5,op_0
	bgeu	t4,a6,.+8
	jal	x0,.+12
	addi	a6,x0,0
	jal	x0,.+8
	addi	a6,x0,1
	sub	op_0,x0,a6
	sub	a6,t3,op_0
	sub	op_0,x0,a6
	sub	a6,t6,op_0
	bgeu	a6,t3,.+8
	jal	x0,.L22
	bne	t3,a6,.+8
	jal	x0,.L23
.L14:
	or	op_0,t1,a7
	or	op_3,t1,a7
	sub	op_2,op_3,a7
	sub	op_1,t1,op_2
	sub	a2,op_0,op_1
	or	op_0,a0,a1
	or	op_3,a0,a1
	sub	op_2,op_3,a1
	sub	op_1,a0,op_2
	sub	a3,op_0,op_1
	or	a3,a3,a2
	addi	op_0,x0,1
	bgeu	a3,op_0,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	addi	a2,zero,1
	bgeu	a6,t3,.+8
	jal	x0,.L16
	bne	t3,a6,.+8
	jal	x0,.L24
	addi	a2,zero,0
.L16:
	or	op_2,a2,a3
	or	op_5,a2,a3
	sub	op_4,op_5,a3
	sub	op_3,a2,op_4
	sub	op_1,op_2,op_3
	or	op_6,a3,a2
	or	op_0,op_1,op_6
	or	op_9,a2,a3
	or	op_12,a2,a3
	sub	op_11,op_12,a3
	sub	op_10,a2,op_11
	sub	op_8,op_9,op_10
	or	op_16,a2,a3
	or	op_19,a2,a3
	sub	op_18,op_19,a3
	sub	op_17,a2,op_18
	sub	op_15,op_16,op_17
	or	op_20,a3,a2
	or	op_14,op_15,op_20
	or	op_21,a3,a2
	sub	op_13,op_14,op_21
	sub	op_7,op_8,op_13
	sub	a3,op_0,op_7
	addi	a2,zero,1
	bgeu	a7,t1,.+8
	jal	x0,.L18
	bne	t1,a7,.+8
	jal	x0,.L25
	addi	a2,zero,0
.L18:
	addi	a0,zero,1
	bgeu	a7,a5,.+8
	jal	x0,.L21
	bne	a5,a7,.+8
	jal	x0,.L26
.L20:
	or	a2,a2,a0
	or	a3,a3,a2
	addi	op_0,x0,31
	sll	a2,a3,op_0
	addi	op_0,x0,31
	sra	a0,a2,op_0
	or	op_2,a4,a0
	or	op_5,a4,a0
	sub	op_4,op_5,a0
	sub	op_3,a4,op_4
	sub	op_1,op_2,op_3
	or	op_6,a0,a4
	or	op_0,op_1,op_6
	or	op_9,a4,a0
	or	op_12,a4,a0
	sub	op_11,op_12,a0
	sub	op_10,a4,op_11
	sub	op_8,op_9,op_10
	or	op_16,a4,a0
	or	op_19,a4,a0
	sub	op_18,op_19,a0
	sub	op_17,a4,op_18
	sub	op_15,op_16,op_17
	or	op_20,a0,a4
	or	op_14,op_15,op_20
	or	op_21,a0,a4
	sub	op_13,op_14,op_21
	sub	op_7,op_8,op_13
	sub	a0,op_0,op_7
	addi	op_0,x0,31
	sll	a4,a3,op_0
	addi	op_0,x0,31
	sra	a3,a4,op_0
	sub	a0,a1,a0
	or	op_2,a5,a3
	or	op_5,a5,a3
	sub	op_4,op_5,a3
	sub	op_3,a5,op_4
	sub	op_1,op_2,op_3
	or	op_6,a3,a5
	or	op_0,op_1,op_6
	or	op_9,a5,a3
	or	op_12,a5,a3
	sub	op_11,op_12,a3
	sub	op_10,a5,op_11
	sub	op_8,op_9,op_10
	or	op_16,a5,a3
	or	op_19,a5,a3
	sub	op_18,op_19,a3
	sub	op_17,a5,op_18
	sub	op_15,op_16,op_17
	or	op_20,a3,a5
	or	op_14,op_15,op_20
	or	op_21,a3,a5
	sub	op_13,op_14,op_21
	sub	op_7,op_8,op_13
	sub	a3,op_0,op_7
	bgeu	a1,a0,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	sub	a7,a7,a3
	sub	a1,a7,a1
	jalr	zero,ra,0
.L26:
	bgeu	a1,a4,.L20
.L21:
	addi	a0,zero,0
	jal	x0,.L20
.L23:
	bgeu	t4,t5,.L14
.L22:
	addi	a1,a3,1
	bgeu	a1,a3,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	sub	op_0,x0,a3
	sub	a7,a2,op_0
	jal	x0,.L14
.L24:
	bgeu	t4,t5,.+8
	jal	x0,.L16
	addi	a2,zero,0
	jal	x0,.L16
.L25:
	bgeu	a1,a0,.+8
	jal	x0,.L18
	addi	a2,zero,0
	jal	x0,.L18
	.size	montmul, .-montmul
	.align	2
	.type	benchmark_body.constprop.0, @function
benchmark_body.constprop.0:
	addi	sp,sp,-112
	lui	a5,%hi(in_b)
	lui	a4,%hi(in_a)
	sw	s8,72(sp)
	sw	s9,68(sp)
	lw	s8,%lo(in_b)(a5)
	lw	s9,%lo(in_a)(a4)
	sw	s11,60(sp)
	lw	s11,%lo(in_a+4)(a4)
	sw	s10,64(sp)
	lw	s10,%lo(in_b+4)(a5)
	lui	a5,%hi(in_m)
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,s9,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,s9,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,s8,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,s8,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,s9,op_25
	addi	op_29,x0,16
	srl	op_28,s9,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,s8,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,s8,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,s9,op_46
	addi	op_50,x0,16
	srl	op_49,s9,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,s8,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,s8,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,s9,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,s9,op_70
	addi	op_78,x0,16
	srl	op_77,s8,op_78
	addi	op_82,x0,16
	srl	op_81,s8,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,s9,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,s9,op_91
	addi	op_99,x0,16
	srl	op_98,s8,op_99
	addi	op_103,x0,16
	srl	op_102,s8,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,s9,op_119
	addi	op_123,x0,16
	srl	op_122,s9,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,s8,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,s8,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,s9,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,s9,op_141
	addi	op_149,x0,16
	srl	op_148,s8,op_149
	addi	op_153,x0,16
	srl	op_152,s8,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,s9,op_163
	addi	op_167,x0,16
	srl	op_166,s9,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,s8,op_174
	addi	op_178,x0,16
	srl	op_177,s8,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a3,op_0,op_113
	sw	s0,104(sp)
	sw	s4,88(sp)
	lw	s0,%lo(in_m+4)(a5)
	lw	s4,%lo(in_m)(a5)
	sw	s3,92(sp)
	sw	s6,80(sp)
	sw	s7,76(sp)
	sw	ra,108(sp)
	sw	s1,100(sp)
	callmul	a5,s11,s8
	sw	s2,96(sp)
	sw	s5,84(sp)
	addi	s7,zero,423
	addi	s3,zero,64
	lui	s6,524288
	callmul	a4,s9,s10
	sub	op_0,x0,a5
	sub	a3,a3,op_0
	bgeu	a3,a5,.+8
	jal	x0,.+12
	addi	a5,x0,0
	jal	x0,.+8
	addi	a5,x0,1
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,s11,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,s11,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,s8,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,s8,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,s11,op_25
	addi	op_29,x0,16
	srl	op_28,s11,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,s8,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,s8,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,s11,op_46
	addi	op_50,x0,16
	srl	op_49,s11,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,s8,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,s8,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,s11,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,s11,op_70
	addi	op_78,x0,16
	srl	op_77,s8,op_78
	addi	op_82,x0,16
	srl	op_81,s8,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,s11,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,s11,op_91
	addi	op_99,x0,16
	srl	op_98,s8,op_99
	addi	op_103,x0,16
	srl	op_102,s8,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,s11,op_119
	addi	op_123,x0,16
	srl	op_122,s11,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,s8,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,s8,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,s11,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,s11,op_141
	addi	op_149,x0,16
	srl	op_148,s8,op_149
	addi	op_153,x0,16
	srl	op_152,s8,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,s11,op_163
	addi	op_167,x0,16
	srl	op_166,s11,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,s8,op_174
	addi	op_178,x0,16
	srl	op_177,s8,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a2,op_0,op_113
	sub	op_0,x0,a4
	sub	t1,a3,op_0
	bgeu	t1,a4,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	sw	t1,24(sp)
	callmul	a4,s11,s10
	sub	op_0,x0,a5
	sub	a5,a2,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,s11,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,s11,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,s10,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,s10,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,s11,op_25
	addi	op_29,x0,16
	srl	op_28,s11,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,s10,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,s10,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,s11,op_46
	addi	op_50,x0,16
	srl	op_49,s11,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,s10,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,s10,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,s11,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,s11,op_70
	addi	op_78,x0,16
	srl	op_77,s10,op_78
	addi	op_82,x0,16
	srl	op_81,s10,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,s11,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,s11,op_91
	addi	op_99,x0,16
	srl	op_98,s10,op_99
	addi	op_103,x0,16
	srl	op_102,s10,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,s11,op_119
	addi	op_123,x0,16
	srl	op_122,s11,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,s10,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,s10,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,s11,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,s11,op_141
	addi	op_149,x0,16
	srl	op_148,s10,op_149
	addi	op_153,x0,16
	srl	op_152,s10,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,s11,op_163
	addi	op_167,x0,16
	srl	op_166,s11,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,s10,op_174
	addi	op_178,x0,16
	srl	op_177,s10,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a2,op_0,op_113
	sub	op_0,x0,a5
	sub	a4,a4,op_0
	bgeu	a4,a5,.+8
	jal	x0,.+12
	addi	a5,x0,0
	jal	x0,.+8
	addi	a5,x0,1
	sub	op_0,x0,a5
	sub	a5,a2,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,s9,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,s9,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,s10,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,s10,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,s9,op_25
	addi	op_29,x0,16
	srl	op_28,s9,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,s10,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,s10,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,s9,op_46
	addi	op_50,x0,16
	srl	op_49,s9,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,s10,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,s10,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,s9,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,s9,op_70
	addi	op_78,x0,16
	srl	op_77,s10,op_78
	addi	op_82,x0,16
	srl	op_81,s10,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,s9,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,s9,op_91
	addi	op_99,x0,16
	srl	op_98,s10,op_99
	addi	op_103,x0,16
	srl	op_102,s10,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,s9,op_119
	addi	op_123,x0,16
	srl	op_122,s9,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,s10,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,s10,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,s9,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,s9,op_141
	addi	op_149,x0,16
	srl	op_148,s10,op_149
	addi	op_153,x0,16
	srl	op_152,s10,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,s9,op_163
	addi	op_167,x0,16
	srl	op_166,s9,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,s10,op_174
	addi	op_178,x0,16
	srl	op_177,s10,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a2,op_0,op_113
	sub	op_0,x0,a3
	sub	a3,a2,op_0
	callmul	a2,s9,s8
	sub	op_0,x0,a4
	sub	t3,a3,op_0
	bgeu	t3,a4,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	sub	op_0,x0,a4
	sub	a5,a5,op_0
	sw	a5,20(sp)
	sw	t3,28(sp)
	sw	a2,16(sp)
.L49:
	lw	a2,16(sp)
	lw	a6,24(sp)
	lw	a4,28(sp)
	lw	a3,20(sp)
	addi	a7,zero,0
.L30:
	addi	op_0,x0,31
	srl	a1,a4,op_0
	addi	op_0,x0,31
	sra	a0,a3,op_0
	addi	op_0,x0,1
	sll	a5,a3,op_0
	addi	op_0,x0,1
	sll	a4,a4,op_0
	addi	op_0,x0,31
	srl	a3,a6,op_0
	sub	op_0,x0,a1
	sub	a5,a5,op_0
	or	a4,a3,a4
	addi	op_0,x0,1
	sll	a1,a6,op_0
	addi	op_0,x0,31
	srl	a3,a2,op_0
	sub	op_0,x0,a3
	sub	a1,a1,op_0
	or	t1,a0,a5
	addi	a3,a5,0
	addi	a7,a7,1
	addi	op_0,x0,1
	sll	a2,a2,op_0
	or	a0,a0,a4
	addi	a6,a1,0
	bgeu	t1,s0,.+8
	jal	x0,.L28
	sub	t4,a4,s4
	addi	t3,a2,1
	bgeu	a4,t4,.+8
	jal	x0,.+12
	addi	t6,x0,0
	jal	x0,.+8
	addi	t6,x0,1
	bgeu	t3,a2,.+8
	jal	x0,.+12
	addi	t5,x0,0
	jal	x0,.+8
	addi	t5,x0,1
	sub	a5,a5,s0
	bne	s0,t1,.L50
	bgeu	a0,s4,.+8
	jal	x0,.L28
.L50:
	addi	a4,t4,0
	sub	a3,a5,t6
	addi	a2,t3,0
	sub	op_0,x0,t5
	sub	a6,a1,op_0
.L28:
	bne	a7,s3,.L30
	callmul	a2,a3,a4
	addi	a7,zero,0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a4,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a4,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a4,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a4,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a4,op_25
	addi	op_29,x0,16
	srl	op_28,a4,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a4,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a4,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a4,op_46
	addi	op_50,x0,16
	srl	op_49,a4,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a4,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a4,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a4,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a4,op_70
	addi	op_78,x0,16
	srl	op_77,a4,op_78
	addi	op_82,x0,16
	srl	op_81,a4,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a4,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a4,op_91
	addi	op_99,x0,16
	srl	op_98,a4,op_99
	addi	op_103,x0,16
	srl	op_102,a4,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a4,op_119
	addi	op_123,x0,16
	srl	op_122,a4,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a4,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a4,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a4,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a4,op_141
	addi	op_149,x0,16
	srl	op_148,a4,op_149
	addi	op_153,x0,16
	srl	op_152,a4,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a4,op_163
	addi	op_167,x0,16
	srl	op_166,a4,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a4,op_174
	addi	op_178,x0,16
	srl	op_177,a4,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a5,op_0,op_113
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a3,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a3,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a4,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a4,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a3,op_25
	addi	op_29,x0,16
	srl	op_28,a3,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a4,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a4,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a3,op_46
	addi	op_50,x0,16
	srl	op_49,a3,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a4,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a4,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a3,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a3,op_70
	addi	op_78,x0,16
	srl	op_77,a4,op_78
	addi	op_82,x0,16
	srl	op_81,a4,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a3,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a3,op_91
	addi	op_99,x0,16
	srl	op_98,a4,op_99
	addi	op_103,x0,16
	srl	op_102,a4,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a3,op_119
	addi	op_123,x0,16
	srl	op_122,a3,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a4,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a4,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a3,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a3,op_141
	addi	op_149,x0,16
	srl	op_148,a4,op_149
	addi	op_153,x0,16
	srl	op_152,a4,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a3,op_163
	addi	op_167,x0,16
	srl	op_166,a3,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a4,op_174
	addi	op_178,x0,16
	srl	op_177,a4,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	t1,op_0,op_113
	sub	op_0,x0,a5
	sub	a6,a2,op_0
	bgeu	a6,a5,.+8
	jal	x0,.+12
	addi	a5,x0,0
	jal	x0,.+8
	addi	a5,x0,1
	sub	op_0,x0,a2
	sub	a6,a6,op_0
	bgeu	a6,a2,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	callmul	a0,a3,a3
	sub	op_0,x0,a5
	sub	a2,t1,op_0
	sub	op_0,x0,a1
	sub	a5,t1,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a3,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a3,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a3,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a3,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a3,op_25
	addi	op_29,x0,16
	srl	op_28,a3,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a3,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a3,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a3,op_46
	addi	op_50,x0,16
	srl	op_49,a3,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a3,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a3,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a3,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a3,op_70
	addi	op_78,x0,16
	srl	op_77,a3,op_78
	addi	op_82,x0,16
	srl	op_81,a3,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a3,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a3,op_91
	addi	op_99,x0,16
	srl	op_98,a3,op_99
	addi	op_103,x0,16
	srl	op_102,a3,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a3,op_119
	addi	op_123,x0,16
	srl	op_122,a3,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a3,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a3,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a3,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a3,op_141
	addi	op_149,x0,16
	srl	op_148,a3,op_149
	addi	op_153,x0,16
	srl	op_152,a3,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a3,op_163
	addi	op_167,x0,16
	srl	op_166,a3,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a3,op_174
	addi	op_178,x0,16
	srl	op_177,a3,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a3,op_0,op_113
	sub	op_0,x0,a2
	sub	a1,a0,op_0
	bgeu	a1,a2,.+8
	jal	x0,.+12
	addi	a2,x0,0
	jal	x0,.+8
	addi	a2,x0,1
	sub	op_0,x0,a1
	sub	a5,a5,op_0
	bgeu	a5,a1,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	callmul	a4,a4,a4
	sub	op_0,x0,a2
	sub	a3,a3,op_0
	sub	op_0,x0,a1
	sub	a2,a3,op_0
.L33:
	addi	op_0,x0,31
	srl	a1,a5,op_0
	addi	op_0,x0,31
	sra	a0,a2,op_0
	addi	op_0,x0,1
	sll	a3,a2,op_0
	addi	op_0,x0,1
	sll	a5,a5,op_0
	addi	op_0,x0,31
	srl	a2,a6,op_0
	sub	op_0,x0,a1
	sub	a3,a3,op_0
	or	a5,a2,a5
	addi	op_0,x0,1
	sll	a1,a6,op_0
	addi	op_0,x0,31
	srl	a2,a4,op_0
	sub	op_0,x0,a2
	sub	a1,a1,op_0
	or	t1,a0,a3
	addi	a2,a3,0
	addi	a7,a7,1
	addi	op_0,x0,1
	sll	a4,a4,op_0
	or	a0,a0,a5
	addi	a6,a1,0
	bgeu	t1,s0,.+8
	jal	x0,.L31
	sub	t4,a5,s4
	addi	t3,a4,1
	bgeu	a5,t4,.+8
	jal	x0,.+12
	addi	t6,x0,0
	jal	x0,.+8
	addi	t6,x0,1
	bgeu	t3,a4,.+8
	jal	x0,.+12
	addi	t5,x0,0
	jal	x0,.+8
	addi	t5,x0,1
	sub	a3,a3,s0
	bne	s0,t1,.L51
	bgeu	a0,s4,.+8
	jal	x0,.L31
.L51:
	addi	a5,t4,0
	sub	a2,a3,t6
	addi	a4,t3,0
	sub	op_0,x0,t5
	sub	a6,a1,op_0
.L31:
	bne	a7,s3,.L33
	callmul	a3,a2,a5
	addi	s2,zero,0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a5,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a5,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a5,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a5,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a5,op_25
	addi	op_29,x0,16
	srl	op_28,a5,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a5,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a5,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a5,op_46
	addi	op_50,x0,16
	srl	op_49,a5,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a5,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a5,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a5,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a5,op_70
	addi	op_78,x0,16
	srl	op_77,a5,op_78
	addi	op_82,x0,16
	srl	op_81,a5,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a5,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a5,op_91
	addi	op_99,x0,16
	srl	op_98,a5,op_99
	addi	op_103,x0,16
	srl	op_102,a5,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a5,op_119
	addi	op_123,x0,16
	srl	op_122,a5,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a5,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a5,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a5,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a5,op_141
	addi	op_149,x0,16
	srl	op_148,a5,op_149
	addi	op_153,x0,16
	srl	op_152,a5,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a5,op_163
	addi	op_167,x0,16
	srl	op_166,a5,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a5,op_174
	addi	op_178,x0,16
	srl	op_177,a5,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a4,op_0,op_113
	sub	op_0,x0,a4
	sub	a1,a3,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a2,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a2,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a5,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a5,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a2,op_25
	addi	op_29,x0,16
	srl	op_28,a2,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a5,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a5,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a2,op_46
	addi	op_50,x0,16
	srl	op_49,a2,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a5,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a5,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a2,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a2,op_70
	addi	op_78,x0,16
	srl	op_77,a5,op_78
	addi	op_82,x0,16
	srl	op_81,a5,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a2,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a2,op_91
	addi	op_99,x0,16
	srl	op_98,a5,op_99
	addi	op_103,x0,16
	srl	op_102,a5,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a2,op_119
	addi	op_123,x0,16
	srl	op_122,a2,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a5,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a5,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a2,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a2,op_141
	addi	op_149,x0,16
	srl	op_148,a5,op_149
	addi	op_153,x0,16
	srl	op_152,a5,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a2,op_163
	addi	op_167,x0,16
	srl	op_166,a2,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a5,op_174
	addi	op_178,x0,16
	srl	op_177,a5,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a0,op_0,op_113
	bgeu	a1,a4,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	sub	op_0,x0,a3
	sub	a1,a1,op_0
	bgeu	a1,a3,.+8
	jal	x0,.+12
	addi	s1,x0,0
	jal	x0,.+8
	addi	s1,x0,1
	callmul	a3,a2,a2
	sub	op_0,x0,a4
	sub	a4,a0,op_0
	sub	op_0,x0,s1
	sub	s1,a0,op_0
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,a2,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,a2,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a2,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a2,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,a2,op_25
	addi	op_29,x0,16
	srl	op_28,a2,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a2,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a2,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,a2,op_46
	addi	op_50,x0,16
	srl	op_49,a2,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a2,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a2,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,a2,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,a2,op_70
	addi	op_78,x0,16
	srl	op_77,a2,op_78
	addi	op_82,x0,16
	srl	op_81,a2,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,a2,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,a2,op_91
	addi	op_99,x0,16
	srl	op_98,a2,op_99
	addi	op_103,x0,16
	srl	op_102,a2,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,a2,op_119
	addi	op_123,x0,16
	srl	op_122,a2,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a2,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a2,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,a2,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,a2,op_141
	addi	op_149,x0,16
	srl	op_148,a2,op_149
	addi	op_153,x0,16
	srl	op_152,a2,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,a2,op_163
	addi	op_167,x0,16
	srl	op_166,a2,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a2,op_174
	addi	op_178,x0,16
	srl	op_177,a2,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a2,op_0,op_113
	sub	op_0,x0,a4
	sub	a3,a3,op_0
	bgeu	a3,a4,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	sub	op_0,x0,a3
	sub	s1,s1,op_0
	bgeu	s1,a3,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	callmul	a5,a5,a5
	sub	op_0,x0,a4
	sub	a4,a2,op_0
	sub	op_0,x0,a3
	sub	s5,a4,op_0
.L36:
	addi	op_0,x0,31
	srl	a3,s1,op_0
	addi	op_0,x0,1
	sll	a4,s5,op_0
	sub	op_0,x0,a3
	sub	a4,a4,op_0
	addi	op_0,x0,1
	sll	s1,s1,op_0
	addi	op_0,x0,31
	srl	a3,a1,op_0
	addi	op_0,x0,31
	sra	a2,s5,op_0
	or	s1,a3,s1
	addi	op_0,x0,1
	sll	a3,a1,op_0
	addi	op_0,x0,31
	srl	a1,a5,op_0
	sub	op_0,x0,a1
	sub	a3,a3,op_0
	or	a0,a2,a4
	addi	s5,a4,0
	addi	s2,s2,1
	addi	op_0,x0,1
	sll	a5,a5,op_0
	or	a2,a2,s1
	addi	a1,a3,0
	bgeu	a0,s0,.+8
	jal	x0,.L34
	sub	a7,s1,s4
	addi	a6,a5,1
	bgeu	s1,a7,.+8
	jal	x0,.+12
	addi	t3,x0,0
	jal	x0,.+8
	addi	t3,x0,1
	bgeu	a6,a5,.+8
	jal	x0,.+12
	addi	t1,x0,0
	jal	x0,.+8
	addi	t1,x0,1
	sub	a4,a4,s0
	bne	s0,a0,.L52
	bgeu	a2,s4,.+8
	jal	x0,.L34
.L52:
	addi	s1,a7,0
	sub	s5,a4,t3
	addi	a5,a6,0
	sub	op_0,x0,t1
	sub	a1,a3,op_0
.L34:
	bne	s2,s3,.L36
	addi	a2,zero,0
	addi	a3,zero,0
	addi	a4,zero,1
	addi	a5,zero,0
	jal	x0,.L39
.L66:
	addi	s2,s2,-1
	sub	op_0,x0,t6
	sub	a4,t5,op_0
	addi	a5,t4,0
	bne	s2,zero,.+8
	jal	x0,.L65
.L39:
	or	op_0,a5,s0
	or	op_3,a5,s0
	sub	op_2,op_3,s0
	sub	op_1,a5,op_2
	sub	a6,op_0,op_1
	or	op_0,a4,s4
	or	op_3,a4,s4
	sub	op_2,op_3,s4
	sub	op_1,a4,op_2
	sub	a1,op_0,op_1
	addi	op_0,x0,31
	sll	a7,a6,op_0
	addi	op_0,x0,1
	srl	a1,a1,op_0
	addi	op_0,x0,31
	sll	t5,a3,op_0
	sub	op_0,x0,a7
	sub	a1,a1,op_0
	addi	op_0,x0,1
	srl	t4,a2,op_0
	addi	op_0,x0,1
	srl	t3,a3,op_0
	or	op_1,s4,a4
	or	op_0,a1,op_1
	or	op_5,s4,a4
	or	op_4,a1,op_5
	or	op_6,s4,a4
	sub	op_3,op_4,op_6
	sub	op_2,a1,op_3
	sub	a0,op_0,op_2
	addi	op_2,x0,1
	or	op_1,a4,op_2
	addi	op_3,x0,1
	sub	op_0,op_1,op_3
	sub	t1,a4,op_0
	sub	op_0,x0,t5
	sub	a2,t4,op_0
	addi	a3,t3,0
	or	op_1,s0,a5
	or	op_0,a6,op_1
	or	op_5,s0,a5
	or	op_4,a6,op_5
	or	op_6,s0,a5
	sub	op_3,op_4,op_6
	sub	op_2,a6,op_3
	sub	a7,op_0,op_2
	sub	op_0,x0,a0
	sub	a1,a1,op_0
	addi	op_0,x0,1
	srl	a6,a6,op_0
	addi	op_0,x0,1
	srl	t4,a5,op_0
	addi	op_0,x0,31
	sll	t6,a5,op_0
	addi	op_0,x0,1
	srl	t5,a4,op_0
	sub	op_0,x0,t3
	sub	t3,s6,op_0
	bne	t1,zero,.+8
	jal	x0,.L66
	bgeu	a1,a0,.+8
	jal	x0,.+12
	addi	a0,x0,0
	jal	x0,.+8
	addi	a0,x0,1
	sub	op_0,x0,a7
	sub	a7,a6,op_0
	addi	s2,s2,-1
	addi	a4,a1,0
	sub	op_0,x0,a0
	sub	a5,a7,op_0
	addi	a3,t3,0
	bne	s2,zero,.L39
.L65:
	sw	a4,32(sp)
	sw	a5,36(sp)
	sw	a2,40(sp)
	sw	a3,44(sp)
	lw	a4,32(sp)
	lw	a5,36(sp)
	lw	a5,40(sp)
	lw	a6,44(sp)
	addi	a7,zero,0
	sw	a5,8(sp)
	sw	a6,12(sp)
	addi	a0,s9,0
	addi	a1,s11,0
	addi	a3,zero,0
	addi	a4,zero,0
.L42:
	addi	op_0,x0,31
	srl	a2,a0,op_0
	addi	op_0,x0,1
	sll	a5,a1,op_0
	sub	op_0,x0,a2
	sub	a5,a5,op_0
	addi	op_0,x0,1
	sll	a0,a0,op_0
	addi	op_0,x0,31
	srl	a2,a4,op_0
	addi	op_0,x0,31
	sra	a6,a1,op_0
	or	a0,a2,a0
	addi	op_0,x0,1
	sll	a2,a4,op_0
	addi	op_0,x0,31
	srl	a4,a3,op_0
	sub	op_0,x0,a4
	sub	a2,a2,op_0
	or	t1,a6,a5
	addi	a1,a5,0
	addi	a7,a7,1
	addi	op_0,x0,1
	sll	a3,a3,op_0
	or	a6,a6,a0
	addi	a4,a2,0
	bgeu	t1,s0,.+8
	jal	x0,.L40
	sub	t3,a0,s4
	bne	s0,t1,.L53
	bgeu	a6,s4,.+8
	jal	x0,.L40
.L53:
	addi	a6,a3,1
	bgeu	a0,t3,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	bgeu	a6,a3,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	sub	a5,a5,s0
	addi	a0,t3,0
	sub	a1,a5,a1
	sub	op_0,x0,a4
	sub	a4,a2,op_0
	addi	a3,a6,0
.L40:
	bne	a7,s3,.L42
	addi	t3,zero,0
	addi	a2,s8,0
	addi	a3,s10,0
	addi	a6,zero,0
	addi	a4,zero,0
.L45:
	addi	op_0,x0,31
	srl	a7,a2,op_0
	addi	op_0,x0,31
	sra	t1,a3,op_0
	addi	op_0,x0,1
	sll	a5,a3,op_0
	addi	op_0,x0,1
	sll	a2,a2,op_0
	addi	op_0,x0,31
	srl	a3,a4,op_0
	sub	op_0,x0,a7
	sub	a5,a5,op_0
	or	a2,a3,a2
	addi	op_0,x0,1
	sll	a7,a4,op_0
	addi	op_0,x0,31
	srl	a3,a6,op_0
	sub	op_0,x0,a3
	sub	a7,a7,op_0
	or	t4,t1,a5
	addi	a3,a5,0
	addi	t3,t3,1
	addi	op_0,x0,1
	sll	a6,a6,op_0
	or	t1,t1,a2
	addi	a4,a7,0
	bgeu	t4,s0,.+8
	jal	x0,.L43
	sub	t5,a2,s4
	bne	s0,t4,.L54
	bgeu	t1,s4,.+8
	jal	x0,.L43
.L54:
	bgeu	a2,t5,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	sub	a5,a5,s0
	sub	a3,a5,a3
	addi	a5,a6,1
	bgeu	a5,a6,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	addi	a2,t5,0
	sub	op_0,x0,a4
	sub	a4,a7,op_0
	addi	a6,a5,0
.L43:
	bne	t3,s3,.L45
	lw	a6,40(sp)
	lw	a7,44(sp)
	addi	a4,s4,0
	addi	a5,s0,0
.Lpcrel_1:
	auipc	ra,%pcrel_hi(montmul)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_1)
	lw	a6,40(sp)
	lw	a7,44(sp)
	addi	a2,a0,0
	addi	a3,a1,0
	addi	a4,s4,0
	addi	a5,s0,0
.Lpcrel_2:
	auipc	ra,%pcrel_hi(montmul)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_2)
	lw	a6,40(sp)
	lw	a7,44(sp)
	addi	a2,a0,0
	addi	a3,a1,0
	addi	a4,s4,0
	addi	a5,s0,0
.Lpcrel_3:
	auipc	ra,%pcrel_hi(montmul)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_3)
	lw	t1,32(sp)
	lw	t2,36(sp)
	callmul	a7,a1,t1
	addi	op_9,x0,-1
	or	op_8,x0,op_9
	lui	op_11,16
	sub	op_10,x0,op_11
	sub	op_7,op_8,op_10
	or	op_6,a0,op_7
	addi	op_14,x0,-1
	or	op_13,x0,op_14
	lui	op_16,16
	sub	op_15,x0,op_16
	sub	op_12,op_13,op_15
	sub	op_5,op_6,op_12
	sub	op_4,a0,op_5
	addi	op_22,x0,-1
	or	op_21,x0,op_22
	lui	op_24,16
	sub	op_23,x0,op_24
	sub	op_20,op_21,op_23
	or	op_19,t1,op_20
	addi	op_27,x0,-1
	or	op_26,x0,op_27
	lui	op_29,16
	sub	op_28,x0,op_29
	sub	op_25,op_26,op_28
	sub	op_18,op_19,op_25
	sub	op_17,t1,op_18
	callmul	op_3,op_4,op_17
	addi	op_30,x0,16
	srl	op_2,op_3,op_30
	addi	op_37,x0,16
	srl	op_36,a0,op_37
	addi	op_41,x0,16
	srl	op_40,a0,op_41
	addi	op_44,x0,-1
	or	op_43,x0,op_44
	lui	op_46,16
	sub	op_45,x0,op_46
	sub	op_42,op_43,op_45
	or	op_39,op_40,op_42
	addi	op_49,x0,-1
	or	op_48,x0,op_49
	lui	op_51,16
	sub	op_50,x0,op_51
	sub	op_47,op_48,op_50
	sub	op_38,op_39,op_47
	sub	op_35,op_36,op_38
	addi	op_57,x0,-1
	or	op_56,x0,op_57
	lui	op_59,16
	sub	op_58,x0,op_59
	sub	op_55,op_56,op_58
	or	op_54,t1,op_55
	addi	op_62,x0,-1
	or	op_61,x0,op_62
	lui	op_64,16
	sub	op_63,x0,op_64
	sub	op_60,op_61,op_63
	sub	op_53,op_54,op_60
	sub	op_52,t1,op_53
	callmul	op_34,op_35,op_52
	addi	op_70,x0,16
	srl	op_69,a0,op_70
	addi	op_74,x0,16
	srl	op_73,a0,op_74
	addi	op_77,x0,-1
	or	op_76,x0,op_77
	lui	op_79,16
	sub	op_78,x0,op_79
	sub	op_75,op_76,op_78
	or	op_72,op_73,op_75
	addi	op_82,x0,-1
	or	op_81,x0,op_82
	lui	op_84,16
	sub	op_83,x0,op_84
	sub	op_80,op_81,op_83
	sub	op_71,op_72,op_80
	sub	op_68,op_69,op_71
	addi	op_90,x0,-1
	or	op_89,x0,op_90
	lui	op_92,16
	sub	op_91,x0,op_92
	sub	op_88,op_89,op_91
	or	op_87,t1,op_88
	addi	op_95,x0,-1
	or	op_94,x0,op_95
	lui	op_97,16
	sub	op_96,x0,op_97
	sub	op_93,op_94,op_96
	sub	op_86,op_87,op_93
	sub	op_85,t1,op_86
	callmul	op_67,op_68,op_85
	addi	op_100,x0,-1
	or	op_99,x0,op_100
	lui	op_102,16
	sub	op_101,x0,op_102
	sub	op_98,op_99,op_101
	or	op_66,op_67,op_98
	addi	op_105,x0,-1
	or	op_104,x0,op_105
	lui	op_107,16
	sub	op_106,x0,op_107
	sub	op_103,op_104,op_106
	sub	op_65,op_66,op_103
	sub	op_33,op_34,op_65
	addi	op_116,x0,-1
	or	op_115,x0,op_116
	lui	op_118,16
	sub	op_117,x0,op_118
	sub	op_114,op_115,op_117
	or	op_113,a0,op_114
	addi	op_121,x0,-1
	or	op_120,x0,op_121
	lui	op_123,16
	sub	op_122,x0,op_123
	sub	op_119,op_120,op_122
	sub	op_112,op_113,op_119
	sub	op_111,a0,op_112
	addi	op_126,x0,16
	srl	op_125,t1,op_126
	addi	op_130,x0,16
	srl	op_129,t1,op_130
	addi	op_133,x0,-1
	or	op_132,x0,op_133
	lui	op_135,16
	sub	op_134,x0,op_135
	sub	op_131,op_132,op_134
	or	op_128,op_129,op_131
	addi	op_138,x0,-1
	or	op_137,x0,op_138
	lui	op_140,16
	sub	op_139,x0,op_140
	sub	op_136,op_137,op_139
	sub	op_127,op_128,op_136
	sub	op_124,op_125,op_127
	callmul	op_110,op_111,op_124
	addi	op_149,x0,-1
	or	op_148,x0,op_149
	lui	op_151,16
	sub	op_150,x0,op_151
	sub	op_147,op_148,op_150
	or	op_146,a0,op_147
	addi	op_154,x0,-1
	or	op_153,x0,op_154
	lui	op_156,16
	sub	op_155,x0,op_156
	sub	op_152,op_153,op_155
	sub	op_145,op_146,op_152
	sub	op_144,a0,op_145
	addi	op_159,x0,16
	srl	op_158,t1,op_159
	addi	op_163,x0,16
	srl	op_162,t1,op_163
	addi	op_166,x0,-1
	or	op_165,x0,op_166
	lui	op_168,16
	sub	op_167,x0,op_168
	sub	op_164,op_165,op_167
	or	op_161,op_162,op_164
	addi	op_171,x0,-1
	or	op_170,x0,op_171
	lui	op_173,16
	sub	op_172,x0,op_173
	sub	op_169,op_170,op_172
	sub	op_160,op_161,op_169
	sub	op_157,op_158,op_160
	callmul	op_143,op_144,op_157
	addi	op_176,x0,-1
	or	op_175,x0,op_176
	lui	op_178,16
	sub	op_177,x0,op_178
	sub	op_174,op_175,op_177
	or	op_142,op_143,op_174
	addi	op_181,x0,-1
	or	op_180,x0,op_181
	lui	op_183,16
	sub	op_182,x0,op_183
	sub	op_179,op_180,op_182
	sub	op_141,op_142,op_179
	sub	op_109,op_110,op_141
	sub	op_108,x0,op_109
	sub	op_32,op_33,op_108
	sub	op_31,x0,op_32
	sub	op_1,op_2,op_31
	addi	op_184,x0,16
	srl	op_0,op_1,op_184
	addi	op_191,x0,16
	srl	op_190,a0,op_191
	addi	op_195,x0,16
	srl	op_194,a0,op_195
	addi	op_198,x0,-1
	or	op_197,x0,op_198
	lui	op_200,16
	sub	op_199,x0,op_200
	sub	op_196,op_197,op_199
	or	op_193,op_194,op_196
	addi	op_203,x0,-1
	or	op_202,x0,op_203
	lui	op_205,16
	sub	op_204,x0,op_205
	sub	op_201,op_202,op_204
	sub	op_192,op_193,op_201
	sub	op_189,op_190,op_192
	addi	op_211,x0,-1
	or	op_210,x0,op_211
	lui	op_213,16
	sub	op_212,x0,op_213
	sub	op_209,op_210,op_212
	or	op_208,t1,op_209
	addi	op_216,x0,-1
	or	op_215,x0,op_216
	lui	op_218,16
	sub	op_217,x0,op_218
	sub	op_214,op_215,op_217
	sub	op_207,op_208,op_214
	sub	op_206,t1,op_207
	callmul	op_188,op_189,op_206
	addi	op_219,x0,16
	srl	op_187,op_188,op_219
	addi	op_229,x0,-1
	or	op_228,x0,op_229
	lui	op_231,16
	sub	op_230,x0,op_231
	sub	op_227,op_228,op_230
	or	op_226,a0,op_227
	addi	op_234,x0,-1
	or	op_233,x0,op_234
	lui	op_236,16
	sub	op_235,x0,op_236
	sub	op_232,op_233,op_235
	sub	op_225,op_226,op_232
	sub	op_224,a0,op_225
	addi	op_239,x0,16
	srl	op_238,t1,op_239
	addi	op_243,x0,16
	srl	op_242,t1,op_243
	addi	op_246,x0,-1
	or	op_245,x0,op_246
	lui	op_248,16
	sub	op_247,x0,op_248
	sub	op_244,op_245,op_247
	or	op_241,op_242,op_244
	addi	op_251,x0,-1
	or	op_250,x0,op_251
	lui	op_253,16
	sub	op_252,x0,op_253
	sub	op_249,op_250,op_252
	sub	op_240,op_241,op_249
	sub	op_237,op_238,op_240
	callmul	op_223,op_224,op_237
	addi	op_254,x0,16
	srl	op_222,op_223,op_254
	addi	op_259,x0,16
	srl	op_258,a0,op_259
	addi	op_263,x0,16
	srl	op_262,a0,op_263
	addi	op_266,x0,-1
	or	op_265,x0,op_266
	lui	op_268,16
	sub	op_267,x0,op_268
	sub	op_264,op_265,op_267
	or	op_261,op_262,op_264
	addi	op_271,x0,-1
	or	op_270,x0,op_271
	lui	op_273,16
	sub	op_272,x0,op_273
	sub	op_269,op_270,op_272
	sub	op_260,op_261,op_269
	sub	op_257,op_258,op_260
	addi	op_276,x0,16
	srl	op_275,t1,op_276
	addi	op_280,x0,16
	srl	op_279,t1,op_280
	addi	op_283,x0,-1
	or	op_282,x0,op_283
	lui	op_285,16
	sub	op_284,x0,op_285
	sub	op_281,op_282,op_284
	or	op_278,op_279,op_281
	addi	op_288,x0,-1
	or	op_287,x0,op_288
	lui	op_290,16
	sub	op_289,x0,op_290
	sub	op_286,op_287,op_289
	sub	op_277,op_278,op_286
	sub	op_274,op_275,op_277
	callmul	op_256,op_257,op_274
	sub	op_255,x0,op_256
	sub	op_221,op_222,op_255
	sub	op_220,x0,op_221
	sub	op_186,op_187,op_220
	sub	op_185,x0,op_186
	sub	a2,op_0,op_185
	addi	op_9,x0,-1
	or	op_8,x0,op_9
	lui	op_11,16
	sub	op_10,x0,op_11
	sub	op_7,op_8,op_10
	or	op_6,a1,op_7
	addi	op_14,x0,-1
	or	op_13,x0,op_14
	lui	op_16,16
	sub	op_15,x0,op_16
	sub	op_12,op_13,op_15
	sub	op_5,op_6,op_12
	sub	op_4,a1,op_5
	addi	op_22,x0,-1
	or	op_21,x0,op_22
	lui	op_24,16
	sub	op_23,x0,op_24
	sub	op_20,op_21,op_23
	or	op_19,t1,op_20
	addi	op_27,x0,-1
	or	op_26,x0,op_27
	lui	op_29,16
	sub	op_28,x0,op_29
	sub	op_25,op_26,op_28
	sub	op_18,op_19,op_25
	sub	op_17,t1,op_18
	callmul	op_3,op_4,op_17
	addi	op_30,x0,16
	srl	op_2,op_3,op_30
	addi	op_37,x0,16
	srl	op_36,a1,op_37
	addi	op_41,x0,16
	srl	op_40,a1,op_41
	addi	op_44,x0,-1
	or	op_43,x0,op_44
	lui	op_46,16
	sub	op_45,x0,op_46
	sub	op_42,op_43,op_45
	or	op_39,op_40,op_42
	addi	op_49,x0,-1
	or	op_48,x0,op_49
	lui	op_51,16
	sub	op_50,x0,op_51
	sub	op_47,op_48,op_50
	sub	op_38,op_39,op_47
	sub	op_35,op_36,op_38
	addi	op_57,x0,-1
	or	op_56,x0,op_57
	lui	op_59,16
	sub	op_58,x0,op_59
	sub	op_55,op_56,op_58
	or	op_54,t1,op_55
	addi	op_62,x0,-1
	or	op_61,x0,op_62
	lui	op_64,16
	sub	op_63,x0,op_64
	sub	op_60,op_61,op_63
	sub	op_53,op_54,op_60
	sub	op_52,t1,op_53
	callmul	op_34,op_35,op_52
	addi	op_70,x0,16
	srl	op_69,a1,op_70
	addi	op_74,x0,16
	srl	op_73,a1,op_74
	addi	op_77,x0,-1
	or	op_76,x0,op_77
	lui	op_79,16
	sub	op_78,x0,op_79
	sub	op_75,op_76,op_78
	or	op_72,op_73,op_75
	addi	op_82,x0,-1
	or	op_81,x0,op_82
	lui	op_84,16
	sub	op_83,x0,op_84
	sub	op_80,op_81,op_83
	sub	op_71,op_72,op_80
	sub	op_68,op_69,op_71
	addi	op_90,x0,-1
	or	op_89,x0,op_90
	lui	op_92,16
	sub	op_91,x0,op_92
	sub	op_88,op_89,op_91
	or	op_87,t1,op_88
	addi	op_95,x0,-1
	or	op_94,x0,op_95
	lui	op_97,16
	sub	op_96,x0,op_97
	sub	op_93,op_94,op_96
	sub	op_86,op_87,op_93
	sub	op_85,t1,op_86
	callmul	op_67,op_68,op_85
	addi	op_100,x0,-1
	or	op_99,x0,op_100
	lui	op_102,16
	sub	op_101,x0,op_102
	sub	op_98,op_99,op_101
	or	op_66,op_67,op_98
	addi	op_105,x0,-1
	or	op_104,x0,op_105
	lui	op_107,16
	sub	op_106,x0,op_107
	sub	op_103,op_104,op_106
	sub	op_65,op_66,op_103
	sub	op_33,op_34,op_65
	addi	op_116,x0,-1
	or	op_115,x0,op_116
	lui	op_118,16
	sub	op_117,x0,op_118
	sub	op_114,op_115,op_117
	or	op_113,a1,op_114
	addi	op_121,x0,-1
	or	op_120,x0,op_121
	lui	op_123,16
	sub	op_122,x0,op_123
	sub	op_119,op_120,op_122
	sub	op_112,op_113,op_119
	sub	op_111,a1,op_112
	addi	op_126,x0,16
	srl	op_125,t1,op_126
	addi	op_130,x0,16
	srl	op_129,t1,op_130
	addi	op_133,x0,-1
	or	op_132,x0,op_133
	lui	op_135,16
	sub	op_134,x0,op_135
	sub	op_131,op_132,op_134
	or	op_128,op_129,op_131
	addi	op_138,x0,-1
	or	op_137,x0,op_138
	lui	op_140,16
	sub	op_139,x0,op_140
	sub	op_136,op_137,op_139
	sub	op_127,op_128,op_136
	sub	op_124,op_125,op_127
	callmul	op_110,op_111,op_124
	addi	op_149,x0,-1
	or	op_148,x0,op_149
	lui	op_151,16
	sub	op_150,x0,op_151
	sub	op_147,op_148,op_150
	or	op_146,a1,op_147
	addi	op_154,x0,-1
	or	op_153,x0,op_154
	lui	op_156,16
	sub	op_155,x0,op_156
	sub	op_152,op_153,op_155
	sub	op_145,op_146,op_152
	sub	op_144,a1,op_145
	addi	op_159,x0,16
	srl	op_158,t1,op_159
	addi	op_163,x0,16
	srl	op_162,t1,op_163
	addi	op_166,x0,-1
	or	op_165,x0,op_166
	lui	op_168,16
	sub	op_167,x0,op_168
	sub	op_164,op_165,op_167
	or	op_161,op_162,op_164
	addi	op_171,x0,-1
	or	op_170,x0,op_171
	lui	op_173,16
	sub	op_172,x0,op_173
	sub	op_169,op_170,op_172
	sub	op_160,op_161,op_169
	sub	op_157,op_158,op_160
	callmul	op_143,op_144,op_157
	addi	op_176,x0,-1
	or	op_175,x0,op_176
	lui	op_178,16
	sub	op_177,x0,op_178
	sub	op_174,op_175,op_177
	or	op_142,op_143,op_174
	addi	op_181,x0,-1
	or	op_180,x0,op_181
	lui	op_183,16
	sub	op_182,x0,op_183
	sub	op_179,op_180,op_182
	sub	op_141,op_142,op_179
	sub	op_109,op_110,op_141
	sub	op_108,x0,op_109
	sub	op_32,op_33,op_108
	sub	op_31,x0,op_32
	sub	op_1,op_2,op_31
	addi	op_184,x0,16
	srl	op_0,op_1,op_184
	addi	op_191,x0,16
	srl	op_190,a1,op_191
	addi	op_195,x0,16
	srl	op_194,a1,op_195
	addi	op_198,x0,-1
	or	op_197,x0,op_198
	lui	op_200,16
	sub	op_199,x0,op_200
	sub	op_196,op_197,op_199
	or	op_193,op_194,op_196
	addi	op_203,x0,-1
	or	op_202,x0,op_203
	lui	op_205,16
	sub	op_204,x0,op_205
	sub	op_201,op_202,op_204
	sub	op_192,op_193,op_201
	sub	op_189,op_190,op_192
	addi	op_211,x0,-1
	or	op_210,x0,op_211
	lui	op_213,16
	sub	op_212,x0,op_213
	sub	op_209,op_210,op_212
	or	op_208,t1,op_209
	addi	op_216,x0,-1
	or	op_215,x0,op_216
	lui	op_218,16
	sub	op_217,x0,op_218
	sub	op_214,op_215,op_217
	sub	op_207,op_208,op_214
	sub	op_206,t1,op_207
	callmul	op_188,op_189,op_206
	addi	op_219,x0,16
	srl	op_187,op_188,op_219
	addi	op_229,x0,-1
	or	op_228,x0,op_229
	lui	op_231,16
	sub	op_230,x0,op_231
	sub	op_227,op_228,op_230
	or	op_226,a1,op_227
	addi	op_234,x0,-1
	or	op_233,x0,op_234
	lui	op_236,16
	sub	op_235,x0,op_236
	sub	op_232,op_233,op_235
	sub	op_225,op_226,op_232
	sub	op_224,a1,op_225
	addi	op_239,x0,16
	srl	op_238,t1,op_239
	addi	op_243,x0,16
	srl	op_242,t1,op_243
	addi	op_246,x0,-1
	or	op_245,x0,op_246
	lui	op_248,16
	sub	op_247,x0,op_248
	sub	op_244,op_245,op_247
	or	op_241,op_242,op_244
	addi	op_251,x0,-1
	or	op_250,x0,op_251
	lui	op_253,16
	sub	op_252,x0,op_253
	sub	op_249,op_250,op_252
	sub	op_240,op_241,op_249
	sub	op_237,op_238,op_240
	callmul	op_223,op_224,op_237
	addi	op_254,x0,16
	srl	op_222,op_223,op_254
	addi	op_259,x0,16
	srl	op_258,a1,op_259
	addi	op_263,x0,16
	srl	op_262,a1,op_263
	addi	op_266,x0,-1
	or	op_265,x0,op_266
	lui	op_268,16
	sub	op_267,x0,op_268
	sub	op_264,op_265,op_267
	or	op_261,op_262,op_264
	addi	op_271,x0,-1
	or	op_270,x0,op_271
	lui	op_273,16
	sub	op_272,x0,op_273
	sub	op_269,op_270,op_272
	sub	op_260,op_261,op_269
	sub	op_257,op_258,op_260
	addi	op_276,x0,16
	srl	op_275,t1,op_276
	addi	op_280,x0,16
	srl	op_279,t1,op_280
	addi	op_283,x0,-1
	or	op_282,x0,op_283
	lui	op_285,16
	sub	op_284,x0,op_285
	sub	op_281,op_282,op_284
	or	op_278,op_279,op_281
	addi	op_288,x0,-1
	or	op_287,x0,op_288
	lui	op_290,16
	sub	op_289,x0,op_290
	sub	op_286,op_287,op_289
	sub	op_277,op_278,op_286
	sub	op_274,op_275,op_277
	callmul	op_256,op_257,op_274
	sub	op_255,x0,op_256
	sub	op_221,op_222,op_255
	sub	op_220,x0,op_221
	sub	op_186,op_187,op_220
	sub	op_185,x0,op_186
	sub	a6,op_0,op_185
	sub	op_0,x0,a7
	sub	a2,a2,op_0
	bgeu	a2,a7,.+8
	jal	x0,.+12
	addi	a7,x0,0
	jal	x0,.+8
	addi	a7,x0,1
	sub	op_0,x0,a7
	sub	a6,a6,op_0
	callmul	a7,a1,t2
	addi	op_9,x0,-1
	or	op_8,x0,op_9
	lui	op_11,16
	sub	op_10,x0,op_11
	sub	op_7,op_8,op_10
	or	op_6,a1,op_7
	addi	op_14,x0,-1
	or	op_13,x0,op_14
	lui	op_16,16
	sub	op_15,x0,op_16
	sub	op_12,op_13,op_15
	sub	op_5,op_6,op_12
	sub	op_4,a1,op_5
	addi	op_22,x0,-1
	or	op_21,x0,op_22
	lui	op_24,16
	sub	op_23,x0,op_24
	sub	op_20,op_21,op_23
	or	op_19,t2,op_20
	addi	op_27,x0,-1
	or	op_26,x0,op_27
	lui	op_29,16
	sub	op_28,x0,op_29
	sub	op_25,op_26,op_28
	sub	op_18,op_19,op_25
	sub	op_17,t2,op_18
	callmul	op_3,op_4,op_17
	addi	op_30,x0,16
	srl	op_2,op_3,op_30
	addi	op_37,x0,16
	srl	op_36,a1,op_37
	addi	op_41,x0,16
	srl	op_40,a1,op_41
	addi	op_44,x0,-1
	or	op_43,x0,op_44
	lui	op_46,16
	sub	op_45,x0,op_46
	sub	op_42,op_43,op_45
	or	op_39,op_40,op_42
	addi	op_49,x0,-1
	or	op_48,x0,op_49
	lui	op_51,16
	sub	op_50,x0,op_51
	sub	op_47,op_48,op_50
	sub	op_38,op_39,op_47
	sub	op_35,op_36,op_38
	addi	op_57,x0,-1
	or	op_56,x0,op_57
	lui	op_59,16
	sub	op_58,x0,op_59
	sub	op_55,op_56,op_58
	or	op_54,t2,op_55
	addi	op_62,x0,-1
	or	op_61,x0,op_62
	lui	op_64,16
	sub	op_63,x0,op_64
	sub	op_60,op_61,op_63
	sub	op_53,op_54,op_60
	sub	op_52,t2,op_53
	callmul	op_34,op_35,op_52
	addi	op_70,x0,16
	srl	op_69,a1,op_70
	addi	op_74,x0,16
	srl	op_73,a1,op_74
	addi	op_77,x0,-1
	or	op_76,x0,op_77
	lui	op_79,16
	sub	op_78,x0,op_79
	sub	op_75,op_76,op_78
	or	op_72,op_73,op_75
	addi	op_82,x0,-1
	or	op_81,x0,op_82
	lui	op_84,16
	sub	op_83,x0,op_84
	sub	op_80,op_81,op_83
	sub	op_71,op_72,op_80
	sub	op_68,op_69,op_71
	addi	op_90,x0,-1
	or	op_89,x0,op_90
	lui	op_92,16
	sub	op_91,x0,op_92
	sub	op_88,op_89,op_91
	or	op_87,t2,op_88
	addi	op_95,x0,-1
	or	op_94,x0,op_95
	lui	op_97,16
	sub	op_96,x0,op_97
	sub	op_93,op_94,op_96
	sub	op_86,op_87,op_93
	sub	op_85,t2,op_86
	callmul	op_67,op_68,op_85
	addi	op_100,x0,-1
	or	op_99,x0,op_100
	lui	op_102,16
	sub	op_101,x0,op_102
	sub	op_98,op_99,op_101
	or	op_66,op_67,op_98
	addi	op_105,x0,-1
	or	op_104,x0,op_105
	lui	op_107,16
	sub	op_106,x0,op_107
	sub	op_103,op_104,op_106
	sub	op_65,op_66,op_103
	sub	op_33,op_34,op_65
	addi	op_116,x0,-1
	or	op_115,x0,op_116
	lui	op_118,16
	sub	op_117,x0,op_118
	sub	op_114,op_115,op_117
	or	op_113,a1,op_114
	addi	op_121,x0,-1
	or	op_120,x0,op_121
	lui	op_123,16
	sub	op_122,x0,op_123
	sub	op_119,op_120,op_122
	sub	op_112,op_113,op_119
	sub	op_111,a1,op_112
	addi	op_126,x0,16
	srl	op_125,t2,op_126
	addi	op_130,x0,16
	srl	op_129,t2,op_130
	addi	op_133,x0,-1
	or	op_132,x0,op_133
	lui	op_135,16
	sub	op_134,x0,op_135
	sub	op_131,op_132,op_134
	or	op_128,op_129,op_131
	addi	op_138,x0,-1
	or	op_137,x0,op_138
	lui	op_140,16
	sub	op_139,x0,op_140
	sub	op_136,op_137,op_139
	sub	op_127,op_128,op_136
	sub	op_124,op_125,op_127
	callmul	op_110,op_111,op_124
	addi	op_149,x0,-1
	or	op_148,x0,op_149
	lui	op_151,16
	sub	op_150,x0,op_151
	sub	op_147,op_148,op_150
	or	op_146,a1,op_147
	addi	op_154,x0,-1
	or	op_153,x0,op_154
	lui	op_156,16
	sub	op_155,x0,op_156
	sub	op_152,op_153,op_155
	sub	op_145,op_146,op_152
	sub	op_144,a1,op_145
	addi	op_159,x0,16
	srl	op_158,t2,op_159
	addi	op_163,x0,16
	srl	op_162,t2,op_163
	addi	op_166,x0,-1
	or	op_165,x0,op_166
	lui	op_168,16
	sub	op_167,x0,op_168
	sub	op_164,op_165,op_167
	or	op_161,op_162,op_164
	addi	op_171,x0,-1
	or	op_170,x0,op_171
	lui	op_173,16
	sub	op_172,x0,op_173
	sub	op_169,op_170,op_172
	sub	op_160,op_161,op_169
	sub	op_157,op_158,op_160
	callmul	op_143,op_144,op_157
	addi	op_176,x0,-1
	or	op_175,x0,op_176
	lui	op_178,16
	sub	op_177,x0,op_178
	sub	op_174,op_175,op_177
	or	op_142,op_143,op_174
	addi	op_181,x0,-1
	or	op_180,x0,op_181
	lui	op_183,16
	sub	op_182,x0,op_183
	sub	op_179,op_180,op_182
	sub	op_141,op_142,op_179
	sub	op_109,op_110,op_141
	sub	op_108,x0,op_109
	sub	op_32,op_33,op_108
	sub	op_31,x0,op_32
	sub	op_1,op_2,op_31
	addi	op_184,x0,16
	srl	op_0,op_1,op_184
	addi	op_191,x0,16
	srl	op_190,a1,op_191
	addi	op_195,x0,16
	srl	op_194,a1,op_195
	addi	op_198,x0,-1
	or	op_197,x0,op_198
	lui	op_200,16
	sub	op_199,x0,op_200
	sub	op_196,op_197,op_199
	or	op_193,op_194,op_196
	addi	op_203,x0,-1
	or	op_202,x0,op_203
	lui	op_205,16
	sub	op_204,x0,op_205
	sub	op_201,op_202,op_204
	sub	op_192,op_193,op_201
	sub	op_189,op_190,op_192
	addi	op_211,x0,-1
	or	op_210,x0,op_211
	lui	op_213,16
	sub	op_212,x0,op_213
	sub	op_209,op_210,op_212
	or	op_208,t2,op_209
	addi	op_216,x0,-1
	or	op_215,x0,op_216
	lui	op_218,16
	sub	op_217,x0,op_218
	sub	op_214,op_215,op_217
	sub	op_207,op_208,op_214
	sub	op_206,t2,op_207
	callmul	op_188,op_189,op_206
	addi	op_219,x0,16
	srl	op_187,op_188,op_219
	addi	op_229,x0,-1
	or	op_228,x0,op_229
	lui	op_231,16
	sub	op_230,x0,op_231
	sub	op_227,op_228,op_230
	or	op_226,a1,op_227
	addi	op_234,x0,-1
	or	op_233,x0,op_234
	lui	op_236,16
	sub	op_235,x0,op_236
	sub	op_232,op_233,op_235
	sub	op_225,op_226,op_232
	sub	op_224,a1,op_225
	addi	op_239,x0,16
	srl	op_238,t2,op_239
	addi	op_243,x0,16
	srl	op_242,t2,op_243
	addi	op_246,x0,-1
	or	op_245,x0,op_246
	lui	op_248,16
	sub	op_247,x0,op_248
	sub	op_244,op_245,op_247
	or	op_241,op_242,op_244
	addi	op_251,x0,-1
	or	op_250,x0,op_251
	lui	op_253,16
	sub	op_252,x0,op_253
	sub	op_249,op_250,op_252
	sub	op_240,op_241,op_249
	sub	op_237,op_238,op_240
	callmul	op_223,op_224,op_237
	addi	op_254,x0,16
	srl	op_222,op_223,op_254
	addi	op_259,x0,16
	srl	op_258,a1,op_259
	addi	op_263,x0,16
	srl	op_262,a1,op_263
	addi	op_266,x0,-1
	or	op_265,x0,op_266
	lui	op_268,16
	sub	op_267,x0,op_268
	sub	op_264,op_265,op_267
	or	op_261,op_262,op_264
	addi	op_271,x0,-1
	or	op_270,x0,op_271
	lui	op_273,16
	sub	op_272,x0,op_273
	sub	op_269,op_270,op_272
	sub	op_260,op_261,op_269
	sub	op_257,op_258,op_260
	addi	op_276,x0,16
	srl	op_275,t2,op_276
	addi	op_280,x0,16
	srl	op_279,t2,op_280
	addi	op_283,x0,-1
	or	op_282,x0,op_283
	lui	op_285,16
	sub	op_284,x0,op_285
	sub	op_281,op_282,op_284
	or	op_278,op_279,op_281
	addi	op_288,x0,-1
	or	op_287,x0,op_288
	lui	op_290,16
	sub	op_289,x0,op_290
	sub	op_286,op_287,op_289
	sub	op_277,op_278,op_286
	sub	op_274,op_275,op_277
	callmul	op_256,op_257,op_274
	sub	op_255,x0,op_256
	sub	op_221,op_222,op_255
	sub	op_220,x0,op_221
	sub	op_186,op_187,op_220
	sub	op_185,x0,op_186
	sub	a1,op_0,op_185
	sub	op_0,x0,a6
	sub	a7,a7,op_0
	bgeu	a7,a6,.+8
	jal	x0,.+12
	addi	a6,x0,0
	jal	x0,.+8
	addi	a6,x0,1
	callmul	a3,a0,t2
	sub	op_0,x0,a6
	sub	a6,a1,op_0
	addi	op_9,x0,-1
	or	op_8,x0,op_9
	lui	op_11,16
	sub	op_10,x0,op_11
	sub	op_7,op_8,op_10
	or	op_6,a0,op_7
	addi	op_14,x0,-1
	or	op_13,x0,op_14
	lui	op_16,16
	sub	op_15,x0,op_16
	sub	op_12,op_13,op_15
	sub	op_5,op_6,op_12
	sub	op_4,a0,op_5
	addi	op_22,x0,-1
	or	op_21,x0,op_22
	lui	op_24,16
	sub	op_23,x0,op_24
	sub	op_20,op_21,op_23
	or	op_19,t2,op_20
	addi	op_27,x0,-1
	or	op_26,x0,op_27
	lui	op_29,16
	sub	op_28,x0,op_29
	sub	op_25,op_26,op_28
	sub	op_18,op_19,op_25
	sub	op_17,t2,op_18
	callmul	op_3,op_4,op_17
	addi	op_30,x0,16
	srl	op_2,op_3,op_30
	addi	op_37,x0,16
	srl	op_36,a0,op_37
	addi	op_41,x0,16
	srl	op_40,a0,op_41
	addi	op_44,x0,-1
	or	op_43,x0,op_44
	lui	op_46,16
	sub	op_45,x0,op_46
	sub	op_42,op_43,op_45
	or	op_39,op_40,op_42
	addi	op_49,x0,-1
	or	op_48,x0,op_49
	lui	op_51,16
	sub	op_50,x0,op_51
	sub	op_47,op_48,op_50
	sub	op_38,op_39,op_47
	sub	op_35,op_36,op_38
	addi	op_57,x0,-1
	or	op_56,x0,op_57
	lui	op_59,16
	sub	op_58,x0,op_59
	sub	op_55,op_56,op_58
	or	op_54,t2,op_55
	addi	op_62,x0,-1
	or	op_61,x0,op_62
	lui	op_64,16
	sub	op_63,x0,op_64
	sub	op_60,op_61,op_63
	sub	op_53,op_54,op_60
	sub	op_52,t2,op_53
	callmul	op_34,op_35,op_52
	addi	op_70,x0,16
	srl	op_69,a0,op_70
	addi	op_74,x0,16
	srl	op_73,a0,op_74
	addi	op_77,x0,-1
	or	op_76,x0,op_77
	lui	op_79,16
	sub	op_78,x0,op_79
	sub	op_75,op_76,op_78
	or	op_72,op_73,op_75
	addi	op_82,x0,-1
	or	op_81,x0,op_82
	lui	op_84,16
	sub	op_83,x0,op_84
	sub	op_80,op_81,op_83
	sub	op_71,op_72,op_80
	sub	op_68,op_69,op_71
	addi	op_90,x0,-1
	or	op_89,x0,op_90
	lui	op_92,16
	sub	op_91,x0,op_92
	sub	op_88,op_89,op_91
	or	op_87,t2,op_88
	addi	op_95,x0,-1
	or	op_94,x0,op_95
	lui	op_97,16
	sub	op_96,x0,op_97
	sub	op_93,op_94,op_96
	sub	op_86,op_87,op_93
	sub	op_85,t2,op_86
	callmul	op_67,op_68,op_85
	addi	op_100,x0,-1
	or	op_99,x0,op_100
	lui	op_102,16
	sub	op_101,x0,op_102
	sub	op_98,op_99,op_101
	or	op_66,op_67,op_98
	addi	op_105,x0,-1
	or	op_104,x0,op_105
	lui	op_107,16
	sub	op_106,x0,op_107
	sub	op_103,op_104,op_106
	sub	op_65,op_66,op_103
	sub	op_33,op_34,op_65
	addi	op_116,x0,-1
	or	op_115,x0,op_116
	lui	op_118,16
	sub	op_117,x0,op_118
	sub	op_114,op_115,op_117
	or	op_113,a0,op_114
	addi	op_121,x0,-1
	or	op_120,x0,op_121
	lui	op_123,16
	sub	op_122,x0,op_123
	sub	op_119,op_120,op_122
	sub	op_112,op_113,op_119
	sub	op_111,a0,op_112
	addi	op_126,x0,16
	srl	op_125,t2,op_126
	addi	op_130,x0,16
	srl	op_129,t2,op_130
	addi	op_133,x0,-1
	or	op_132,x0,op_133
	lui	op_135,16
	sub	op_134,x0,op_135
	sub	op_131,op_132,op_134
	or	op_128,op_129,op_131
	addi	op_138,x0,-1
	or	op_137,x0,op_138
	lui	op_140,16
	sub	op_139,x0,op_140
	sub	op_136,op_137,op_139
	sub	op_127,op_128,op_136
	sub	op_124,op_125,op_127
	callmul	op_110,op_111,op_124
	addi	op_149,x0,-1
	or	op_148,x0,op_149
	lui	op_151,16
	sub	op_150,x0,op_151
	sub	op_147,op_148,op_150
	or	op_146,a0,op_147
	addi	op_154,x0,-1
	or	op_153,x0,op_154
	lui	op_156,16
	sub	op_155,x0,op_156
	sub	op_152,op_153,op_155
	sub	op_145,op_146,op_152
	sub	op_144,a0,op_145
	addi	op_159,x0,16
	srl	op_158,t2,op_159
	addi	op_163,x0,16
	srl	op_162,t2,op_163
	addi	op_166,x0,-1
	or	op_165,x0,op_166
	lui	op_168,16
	sub	op_167,x0,op_168
	sub	op_164,op_165,op_167
	or	op_161,op_162,op_164
	addi	op_171,x0,-1
	or	op_170,x0,op_171
	lui	op_173,16
	sub	op_172,x0,op_173
	sub	op_169,op_170,op_172
	sub	op_160,op_161,op_169
	sub	op_157,op_158,op_160
	callmul	op_143,op_144,op_157
	addi	op_176,x0,-1
	or	op_175,x0,op_176
	lui	op_178,16
	sub	op_177,x0,op_178
	sub	op_174,op_175,op_177
	or	op_142,op_143,op_174
	addi	op_181,x0,-1
	or	op_180,x0,op_181
	lui	op_183,16
	sub	op_182,x0,op_183
	sub	op_179,op_180,op_182
	sub	op_141,op_142,op_179
	sub	op_109,op_110,op_141
	sub	op_108,x0,op_109
	sub	op_32,op_33,op_108
	sub	op_31,x0,op_32
	sub	op_1,op_2,op_31
	addi	op_184,x0,16
	srl	op_0,op_1,op_184
	addi	op_191,x0,16
	srl	op_190,a0,op_191
	addi	op_195,x0,16
	srl	op_194,a0,op_195
	addi	op_198,x0,-1
	or	op_197,x0,op_198
	lui	op_200,16
	sub	op_199,x0,op_200
	sub	op_196,op_197,op_199
	or	op_193,op_194,op_196
	addi	op_203,x0,-1
	or	op_202,x0,op_203
	lui	op_205,16
	sub	op_204,x0,op_205
	sub	op_201,op_202,op_204
	sub	op_192,op_193,op_201
	sub	op_189,op_190,op_192
	addi	op_211,x0,-1
	or	op_210,x0,op_211
	lui	op_213,16
	sub	op_212,x0,op_213
	sub	op_209,op_210,op_212
	or	op_208,t2,op_209
	addi	op_216,x0,-1
	or	op_215,x0,op_216
	lui	op_218,16
	sub	op_217,x0,op_218
	sub	op_214,op_215,op_217
	sub	op_207,op_208,op_214
	sub	op_206,t2,op_207
	callmul	op_188,op_189,op_206
	addi	op_219,x0,16
	srl	op_187,op_188,op_219
	addi	op_229,x0,-1
	or	op_228,x0,op_229
	lui	op_231,16
	sub	op_230,x0,op_231
	sub	op_227,op_228,op_230
	or	op_226,a0,op_227
	addi	op_234,x0,-1
	or	op_233,x0,op_234
	lui	op_236,16
	sub	op_235,x0,op_236
	sub	op_232,op_233,op_235
	sub	op_225,op_226,op_232
	sub	op_224,a0,op_225
	addi	op_239,x0,16
	srl	op_238,t2,op_239
	addi	op_243,x0,16
	srl	op_242,t2,op_243
	addi	op_246,x0,-1
	or	op_245,x0,op_246
	lui	op_248,16
	sub	op_247,x0,op_248
	sub	op_244,op_245,op_247
	or	op_241,op_242,op_244
	addi	op_251,x0,-1
	or	op_250,x0,op_251
	lui	op_253,16
	sub	op_252,x0,op_253
	sub	op_249,op_250,op_252
	sub	op_240,op_241,op_249
	sub	op_237,op_238,op_240
	callmul	op_223,op_224,op_237
	addi	op_254,x0,16
	srl	op_222,op_223,op_254
	addi	op_259,x0,16
	srl	op_258,a0,op_259
	addi	op_263,x0,16
	srl	op_262,a0,op_263
	addi	op_266,x0,-1
	or	op_265,x0,op_266
	lui	op_268,16
	sub	op_267,x0,op_268
	sub	op_264,op_265,op_267
	or	op_261,op_262,op_264
	addi	op_271,x0,-1
	or	op_270,x0,op_271
	lui	op_273,16
	sub	op_272,x0,op_273
	sub	op_269,op_270,op_272
	sub	op_260,op_261,op_269
	sub	op_257,op_258,op_260
	addi	op_276,x0,16
	srl	op_275,t2,op_276
	addi	op_280,x0,16
	srl	op_279,t2,op_280
	addi	op_283,x0,-1
	or	op_282,x0,op_283
	lui	op_285,16
	sub	op_284,x0,op_285
	sub	op_281,op_282,op_284
	or	op_278,op_279,op_281
	addi	op_288,x0,-1
	or	op_287,x0,op_288
	lui	op_290,16
	sub	op_289,x0,op_290
	sub	op_286,op_287,op_289
	sub	op_277,op_278,op_286
	sub	op_274,op_275,op_277
	callmul	op_256,op_257,op_274
	sub	op_255,x0,op_256
	sub	op_221,op_222,op_255
	sub	op_220,x0,op_221
	sub	op_186,op_187,op_220
	sub	op_185,x0,op_186
	sub	a5,op_0,op_185
	sub	op_0,x0,a3
	sub	a2,a2,op_0
	bgeu	a2,a3,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	callmul	a1,a0,t1
	sub	op_0,x0,a3
	sub	a4,a5,op_0
	sub	op_0,x0,a7
	sub	a4,a4,op_0
	bgeu	a4,a7,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	sub	op_0,x0,a3
	sub	a3,a6,op_0
.L48:
	addi	op_0,x0,31
	srl	a0,a4,op_0
	addi	op_0,x0,31
	sra	a6,a3,op_0
	addi	op_0,x0,1
	sll	a5,a3,op_0
	addi	op_0,x0,1
	sll	a4,a4,op_0
	addi	op_0,x0,31
	srl	a3,a2,op_0
	sub	op_0,x0,a0
	sub	a5,a5,op_0
	or	a4,a3,a4
	addi	op_0,x0,1
	sll	a0,a2,op_0
	addi	op_0,x0,31
	srl	a3,a1,op_0
	sub	op_0,x0,a3
	sub	a0,a0,op_0
	or	a7,a6,a5
	addi	a3,a5,0
	addi	s2,s2,1
	addi	op_0,x0,1
	sll	a1,a1,op_0
	or	a6,a6,a4
	addi	a2,a0,0
	bgeu	a7,s0,.+8
	jal	x0,.L46
	sub	t1,a4,s4
	bne	s0,a7,.L55
	bgeu	a6,s4,.+8
	jal	x0,.L46
.L55:
	addi	a6,a1,1
	bgeu	a4,t1,.+8
	jal	x0,.+12
	addi	a3,x0,0
	jal	x0,.+8
	addi	a3,x0,1
	bgeu	a6,a1,.+8
	jal	x0,.+12
	addi	a2,x0,0
	jal	x0,.+8
	addi	a2,x0,1
	sub	a5,a5,s0
	addi	a4,t1,0
	sub	a3,a5,a3
	sub	op_0,x0,a2
	sub	a2,a0,op_0
	addi	a1,a6,0
.L46:
	bne	s2,s3,.L48
	addi	s7,s7,-1
	bne	s7,zero,.L49
	lw	a0,8(sp)
	lw	a1,12(sp)
	or	op_0,a3,s5
	or	op_3,a3,s5
	sub	op_2,op_3,s5
	sub	op_1,a3,op_2
	sub	s5,op_0,op_1
	callmul	a5,s0,a0
	lw	ra,108(sp)
	lw	s0,104(sp)
	or	op_0,a4,s1
	or	op_3,a4,s1
	sub	op_2,op_3,s1
	sub	op_1,a4,op_2
	sub	a4,op_0,op_1
	or	a4,a4,s5
	bgeu	zero,a4,.+8
	jal	x0,.+12
	addi	a4,x0,0
	jal	x0,.+8
	addi	a4,x0,1
	lw	s1,100(sp)
	lw	s2,96(sp)
	lw	s3,92(sp)
	lw	s5,84(sp)
	callmul	a2,a1,s4
	lw	s6,80(sp)
	lw	s7,76(sp)
	lw	s8,72(sp)
	lw	s9,68(sp)
	lw	s10,64(sp)
	lw	s11,60(sp)
	lui	op_8,16
	addi	op_7,op_8,-1
	or	op_6,s4,op_7
	lui	op_10,16
	addi	op_9,op_10,-1
	sub	op_5,op_6,op_9
	sub	op_4,s4,op_5
	lui	op_15,16
	addi	op_14,op_15,-1
	or	op_13,a0,op_14
	lui	op_17,16
	addi	op_16,op_17,-1
	sub	op_12,op_13,op_16
	sub	op_11,a0,op_12
	callmul	op_3,op_4,op_11
	addi	op_18,x0,16
	srl	op_2,op_3,op_18
	addi	op_25,x0,16
	srl	op_24,s4,op_25
	addi	op_29,x0,16
	srl	op_28,s4,op_29
	lui	op_31,16
	addi	op_30,op_31,-1
	or	op_27,op_28,op_30
	lui	op_33,16
	addi	op_32,op_33,-1
	sub	op_26,op_27,op_32
	sub	op_23,op_24,op_26
	lui	op_38,16
	addi	op_37,op_38,-1
	or	op_36,a0,op_37
	lui	op_40,16
	addi	op_39,op_40,-1
	sub	op_35,op_36,op_39
	sub	op_34,a0,op_35
	callmul	op_22,op_23,op_34
	addi	op_46,x0,16
	srl	op_45,s4,op_46
	addi	op_50,x0,16
	srl	op_49,s4,op_50
	lui	op_52,16
	addi	op_51,op_52,-1
	or	op_48,op_49,op_51
	lui	op_54,16
	addi	op_53,op_54,-1
	sub	op_47,op_48,op_53
	sub	op_44,op_45,op_47
	lui	op_59,16
	addi	op_58,op_59,-1
	or	op_57,a0,op_58
	lui	op_61,16
	addi	op_60,op_61,-1
	sub	op_56,op_57,op_60
	sub	op_55,a0,op_56
	callmul	op_43,op_44,op_55
	lui	op_63,16
	addi	op_62,op_63,-1
	or	op_42,op_43,op_62
	lui	op_65,16
	addi	op_64,op_65,-1
	sub	op_41,op_42,op_64
	sub	op_21,op_22,op_41
	lui	op_73,16
	addi	op_72,op_73,-1
	or	op_71,s4,op_72
	lui	op_75,16
	addi	op_74,op_75,-1
	sub	op_70,op_71,op_74
	sub	op_69,s4,op_70
	addi	op_78,x0,16
	srl	op_77,a0,op_78
	addi	op_82,x0,16
	srl	op_81,a0,op_82
	lui	op_84,16
	addi	op_83,op_84,-1
	or	op_80,op_81,op_83
	lui	op_86,16
	addi	op_85,op_86,-1
	sub	op_79,op_80,op_85
	sub	op_76,op_77,op_79
	callmul	op_68,op_69,op_76
	lui	op_94,16
	addi	op_93,op_94,-1
	or	op_92,s4,op_93
	lui	op_96,16
	addi	op_95,op_96,-1
	sub	op_91,op_92,op_95
	sub	op_90,s4,op_91
	addi	op_99,x0,16
	srl	op_98,a0,op_99
	addi	op_103,x0,16
	srl	op_102,a0,op_103
	lui	op_105,16
	addi	op_104,op_105,-1
	or	op_101,op_102,op_104
	lui	op_107,16
	addi	op_106,op_107,-1
	sub	op_100,op_101,op_106
	sub	op_97,op_98,op_100
	callmul	op_89,op_90,op_97
	lui	op_109,16
	addi	op_108,op_109,-1
	or	op_88,op_89,op_108
	lui	op_111,16
	addi	op_110,op_111,-1
	sub	op_87,op_88,op_110
	sub	op_67,op_68,op_87
	sub	op_66,x0,op_67
	sub	op_20,op_21,op_66
	sub	op_19,x0,op_20
	sub	op_1,op_2,op_19
	addi	op_112,x0,16
	srl	op_0,op_1,op_112
	addi	op_119,x0,16
	srl	op_118,s4,op_119
	addi	op_123,x0,16
	srl	op_122,s4,op_123
	lui	op_125,16
	addi	op_124,op_125,-1
	or	op_121,op_122,op_124
	lui	op_127,16
	addi	op_126,op_127,-1
	sub	op_120,op_121,op_126
	sub	op_117,op_118,op_120
	lui	op_132,16
	addi	op_131,op_132,-1
	or	op_130,a0,op_131
	lui	op_134,16
	addi	op_133,op_134,-1
	sub	op_129,op_130,op_133
	sub	op_128,a0,op_129
	callmul	op_116,op_117,op_128
	addi	op_135,x0,16
	srl	op_115,op_116,op_135
	lui	op_144,16
	addi	op_143,op_144,-1
	or	op_142,s4,op_143
	lui	op_146,16
	addi	op_145,op_146,-1
	sub	op_141,op_142,op_145
	sub	op_140,s4,op_141
	addi	op_149,x0,16
	srl	op_148,a0,op_149
	addi	op_153,x0,16
	srl	op_152,a0,op_153
	lui	op_155,16
	addi	op_154,op_155,-1
	or	op_151,op_152,op_154
	lui	op_157,16
	addi	op_156,op_157,-1
	sub	op_150,op_151,op_156
	sub	op_147,op_148,op_150
	callmul	op_139,op_140,op_147
	addi	op_158,x0,16
	srl	op_138,op_139,op_158
	addi	op_163,x0,16
	srl	op_162,s4,op_163
	addi	op_167,x0,16
	srl	op_166,s4,op_167
	lui	op_169,16
	addi	op_168,op_169,-1
	or	op_165,op_166,op_168
	lui	op_171,16
	addi	op_170,op_171,-1
	sub	op_164,op_165,op_170
	sub	op_161,op_162,op_164
	addi	op_174,x0,16
	srl	op_173,a0,op_174
	addi	op_178,x0,16
	srl	op_177,a0,op_178
	lui	op_180,16
	addi	op_179,op_180,-1
	or	op_176,op_177,op_179
	lui	op_182,16
	addi	op_181,op_182,-1
	sub	op_175,op_176,op_181
	sub	op_172,op_173,op_175
	callmul	op_160,op_161,op_172
	sub	op_159,x0,op_160
	sub	op_137,op_138,op_159
	sub	op_136,x0,op_137
	sub	op_114,op_115,op_136
	sub	op_113,x0,op_114
	sub	a3,op_0,op_113
	sub	op_0,x0,a5
	sub	a5,a2,op_0
	callmul	a0,s4,a0
	sub	op_0,x0,a5
	sub	a5,a3,op_0
	lw	s4,88(sp)
	addi	sp,sp,112
	or	op_2,a5,a0
	or	op_5,a5,a0
	sub	op_4,op_5,a0
	sub	op_3,a5,op_4
	sub	op_1,op_2,op_3
	or	op_6,a0,a5
	or	op_0,op_1,op_6
	or	op_9,a5,a0
	or	op_12,a5,a0
	sub	op_11,op_12,a0
	sub	op_10,a5,op_11
	sub	op_8,op_9,op_10
	or	op_16,a5,a0
	or	op_19,a5,a0
	sub	op_18,op_19,a0
	sub	op_17,a5,op_18
	sub	op_15,op_16,op_17
	or	op_20,a0,a5
	or	op_14,op_15,op_20
	or	op_21,a0,a5
	sub	op_13,op_14,op_21
	sub	op_7,op_8,op_13
	sub	a0,op_0,op_7
	addi	op_1,x0,1
	or	op_0,x0,op_1
	sub	op_2,x0,a0
	sub	a0,op_0,op_2
	bgeu	zero,a0,.+8
	jal	x0,.+12
	addi	a0,x0,0
	jal	x0,.+8
	addi	a0,x0,1
	or	a0,a0,a4
	jalr	zero,ra,0
	.size	benchmark_body.constprop.0, .-benchmark_body.constprop.0
	.align	2
	.type	benchmark_body.isra.0, @function
benchmark_body.isra.0:
	bge	zero,a0,.L78
	lui	a5,%hi(in_m)
	lw	t4,%lo(in_m)(a5)
	lw	t3,%lo(in_m+4)(a5)
	addi	sp,sp,-32
	sw	s0,28(sp)
	sw	s1,24(sp)
	sw	s2,20(sp)
	sw	s3,16(sp)
	addi	s2,a0,0
	addi	s1,zero,0
	lui	s0,524288
.L73:
	addi	t1,zero,64
	addi	a2,zero,0
	addi	a3,zero,0
	addi	a4,zero,1
	addi	a5,zero,0
	jal	x0,.L72
.L82:
	addi	t1,t1,-1
	addi	a4,a7,0
	sub	op_0,x0,a1
	sub	a5,a6,op_0
	addi	a3,t6,0
	bne	t1,zero,.+8
	jal	x0,.L81
.L72:
	or	op_0,a5,t3
	or	op_3,a5,t3
	sub	op_2,op_3,t3
	sub	op_1,a5,op_2
	sub	a6,op_0,op_1
	or	op_0,a4,t4
	or	op_3,a4,t4
	sub	op_2,op_3,t4
	sub	op_1,a4,op_2
	sub	a1,op_0,op_1
	addi	op_0,x0,31
	sll	a7,a6,op_0
	addi	op_0,x0,1
	srl	a1,a1,op_0
	sub	op_0,x0,a7
	sub	a1,a1,op_0
	or	op_1,t4,a4
	or	op_0,a1,op_1
	or	op_5,t4,a4
	or	op_4,a1,op_5
	or	op_6,t4,a4
	sub	op_3,op_4,op_6
	sub	op_2,a1,op_3
	sub	a7,op_0,op_2
	addi	op_0,x0,31
	sll	a0,a3,op_0
	or	op_1,t3,a5
	or	op_0,a6,op_1
	or	op_5,t3,a5
	or	op_4,a6,op_5
	or	op_6,t3,a5
	sub	op_3,op_4,op_6
	sub	op_2,a6,op_3
	sub	t0,op_0,op_2
	addi	op_0,x0,1
	srl	t2,a2,op_0
	addi	op_0,x0,1
	srl	t6,a3,op_0
	sub	op_0,x0,a1
	sub	a7,a7,op_0
	addi	op_0,x0,1
	srl	a6,a6,op_0
	addi	op_2,x0,1
	or	op_1,a4,op_2
	addi	op_3,x0,1
	sub	op_0,op_1,op_3
	sub	t5,a4,op_0
	sub	op_0,x0,a0
	sub	a2,t2,op_0
	sub	op_0,x0,a6
	sub	a6,t0,op_0
	addi	a3,t6,0
	bgeu	a7,a1,.+8
	jal	x0,.+12
	addi	a1,x0,0
	jal	x0,.+8
	addi	a1,x0,1
	addi	op_0,x0,1
	srl	t0,a5,op_0
	addi	op_0,x0,31
	sll	a0,a5,op_0
	addi	op_0,x0,1
	srl	t2,a4,op_0
	sub	op_0,x0,t6
	sub	t6,s0,op_0
	bne	t5,zero,.L82
	addi	t1,t1,-1
	sub	op_0,x0,a0
	sub	a4,t2,op_0
	addi	a5,t0,0
	bne	t1,zero,.L72
.L81:
	sw	a4,0(sp)
	sw	a5,4(sp)
	sw	a2,8(sp)
	sw	a3,12(sp)
	lw	a4,0(sp)
	lw	a5,4(sp)
	lw	a4,8(sp)
	lw	a5,12(sp)
	lw	a4,8(sp)
	lw	a5,12(sp)
	lw	a4,8(sp)
	lw	a5,12(sp)
	lw	a4,8(sp)
	lw	a5,12(sp)
	lw	a4,0(sp)
	lw	a5,4(sp)
	addi	s1,s1,1
	bne	s2,s1,.L73
	lw	s0,28(sp)
	lw	s1,24(sp)
	lw	s2,20(sp)
	lw	s3,16(sp)
	addi	sp,sp,32
	jalr	zero,ra,0
.L78:
	jalr	zero,ra,0
	.size	benchmark_body.isra.0, .-benchmark_body.isra.0
	.align	2
	.globl	xbinGCD
	.type	xbinGCD, @function
xbinGCD:
	addi	sp,sp,-48
	sw	s0,44(sp)
	sw	s1,40(sp)
	sw	s2,36(sp)
	addi	s0,a1,0
	or	a1,a0,a1
	addi	s2,a4,0
	addi	s1,a5,0
	bne	a1,zero,.+8
	jal	x0,.L89
	sw	s3,32(sp)
	sw	s4,28(sp)
	addi	s3,a2,0
	sw	s5,24(sp)
	sw	s6,20(sp)
	sw	s7,16(sp)
	sw	s8,12(sp)
	addi	s4,a3,0
	addi	a4,a0,0
	addi	a2,s0,0
	addi	t1,zero,0
	addi	t2,zero,0
	addi	a6,zero,1
	addi	a7,zero,0
	jal	x0,.L88
.L93:
	or	t3,t4,t0
	sub	op_0,x0,s8
	sub	a6,s7,op_0
	addi	a7,s6,0
	bne	t3,zero,.+8
	jal	x0,.L92
.L88:
	or	op_0,a7,s4
	or	op_3,a7,s4
	sub	op_2,op_3,s4
	sub	op_1,a7,op_2
	sub	t5,op_0,op_1
	or	op_0,a6,s3
	or	op_3,a6,s3
	sub	op_2,op_3,s3
	sub	op_1,a6,op_2
	sub	t3,op_0,op_1
	addi	op_0,x0,31
	sll	a5,t5,op_0
	addi	op_0,x0,1
	srl	t3,t3,op_0
	sub	op_0,x0,a5
	sub	t3,t3,op_0
	addi	op_0,x0,31
	sll	a1,t2,op_0
	addi	op_0,x0,31
	sll	a5,a2,op_0
	addi	op_0,x0,1
	srl	t4,a4,op_0
	addi	op_0,x0,1
	srl	t0,a2,op_0
	addi	op_0,x0,1
	srl	a2,t1,op_0
	sub	op_0,x0,a1
	sub	t1,a2,op_0
	sub	op_0,x0,a5
	sub	t4,t4,op_0
	or	op_1,s3,a6
	or	op_0,t3,op_1
	or	op_5,s3,a6
	or	op_4,t3,op_5
	or	op_6,s3,a6
	sub	op_3,op_4,op_6
	sub	op_2,t3,op_3
	sub	t6,op_0,op_2
	addi	op_0,x0,1
	srl	a5,t2,op_0
	or	op_1,s4,a7
	sub	op_0,op_1,a7
	sub	a4,s4,op_0
	addi	t2,a5,0
	sub	op_0,x0,t3
	sub	t6,t6,op_0
	addi	op_0,x0,1
	srl	t5,t5,op_0
	sub	op_0,x0,a0
	sub	a5,t1,op_0
	addi	op_2,x0,1
	or	op_1,a6,op_2
	addi	op_3,x0,1
	sub	op_0,op_1,op_3
	sub	a3,a6,op_0
	sub	op_0,x0,t5
	sub	t5,a4,op_0
	addi	op_0,x0,1
	srl	s6,a7,op_0
	addi	op_0,x0,31
	sll	s8,a7,op_0
	addi	op_0,x0,1
	srl	s7,a6,op_0
	bgeu	a5,t1,.+8
	jal	x0,.+12
	addi	s5,x0,0
	jal	x0,.+8
	addi	s5,x0,1
	sub	op_0,x0,s0
	sub	a1,t2,op_0
	bgeu	t6,t3,.+8
	jal	x0,.+12
	addi	t3,x0,0
	jal	x0,.+8
	addi	t3,x0,1
	addi	a4,t4,0
	addi	a2,t0,0
	bne	a3,zero,.+8
	jal	x0,.L93
	sub	op_0,x0,t3
	sub	a7,t5,op_0
	or	t3,t4,t0
	addi	a6,t6,0
	addi	t1,a5,0
	sub	op_0,x0,s5
	sub	t2,a1,op_0
	bne	t3,zero,.L88
.L92:
	lw	s3,32(sp)
	lw	s4,28(sp)
	lw	s5,24(sp)
	lw	s6,20(sp)
	lw	s7,16(sp)
	lw	s8,12(sp)
.L84:
	sw	a6,0(s2)
	sw	a7,4(s2)
	lw	s0,44(sp)
	sw	t1,0(s1)
	sw	t2,4(s1)
	lw	s2,36(sp)
	lw	s1,40(sp)
	addi	sp,sp,48
	jalr	zero,ra,0
.L89:
	addi	t1,a0,0
	addi	t2,s0,0
	addi	a6,zero,1
	addi	a7,zero,0
	jal	x0,.L84
	.size	xbinGCD, .-xbinGCD
	.align	2
	.globl	warm_caches
	.type	warm_caches, @function
warm_caches:
	addi	sp,sp,-16
	sw	ra,12(sp)
.Lpcrel_4:
	auipc	ra,%pcrel_hi(benchmark_body.isra.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_4)
	lw	ra,12(sp)
	addi	sp,sp,16
	jalr	zero,ra,0
	.size	warm_caches, .-warm_caches
	.align	2
	.globl	benchmark
	.type	benchmark, @function
benchmark:
	addi	sp,sp,-16
	sw	ra,12(sp)
.Lpcrel_5:
	auipc	ra,%pcrel_hi(benchmark_body.constprop.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_5)
	lw	ra,12(sp)
	addi	sp,sp,16
	jalr	zero,ra,0
	.size	benchmark, .-benchmark
	.section	.text.startup,"ax",@progbits
	.align	2
	.globl	main
	.type	main, @function
main:
	lui	a3,%hi(.LC0)
	lui	a4,%hi(.LC1)
	lui	a5,%hi(.LC2)
	lw	t1,%lo(.LC0)(a3)
	lw	t2,%lo(.LC0+4)(a3)
	lw	a6,%lo(.LC1)(a4)
	lw	a7,%lo(.LC1+4)(a4)
	lw	a2,%lo(.LC2)(a5)
	lw	a3,%lo(.LC2+4)(a5)
	addi	sp,sp,-32
	lui	a1,%hi(in_m)
	lui	a4,%hi(in_b)
	lui	a5,%hi(in_a)
	addi	a0,zero,1
	sw	ra,28(sp)
	sw	t1,%lo(in_m)(a1)
	sw	t2,%lo(in_m+4)(a1)
	sw	a6,%lo(in_b)(a4)
	sw	a7,%lo(in_b+4)(a4)
	sw	a2,%lo(in_a)(a5)
	sw	a3,%lo(in_a+4)(a5)
.Lpcrel_6:
	auipc	ra,%pcrel_hi(benchmark_body.isra.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_6)
.Lpcrel_7:
	auipc	ra,%pcrel_hi(benchmark)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_7)
	sw	a0,12(sp)
	lw	a0,12(sp)
	lw	ra,28(sp)
	bgeu	zero,a0,.+8
	jal	x0,.+12
	addi	a0,x0,0
	jal	x0,.+8
	addi	a0,x0,1
	addi	sp,sp,32
	jalr	zero,ra,0
	.size	main, .-main
	.text
	.align	2
	.globl	initialise_benchmark
	.type	initialise_benchmark, @function
initialise_benchmark:
	lui	a3,%hi(.LC0)
	lui	a4,%hi(.LC1)
	lui	a5,%hi(.LC2)
	lw	t1,%lo(.LC0)(a3)
	lw	t2,%lo(.LC0+4)(a3)
	lw	a0,%lo(.LC1)(a4)
	lw	a1,%lo(.LC1+4)(a4)
	lw	a2,%lo(.LC2)(a5)
	lw	a3,%lo(.LC2+4)(a5)
	lui	a6,%hi(in_m)
	lui	a4,%hi(in_b)
	lui	a5,%hi(in_a)
	sw	t1,%lo(in_m)(a6)
	sw	t2,%lo(in_m+4)(a6)
	sw	a0,%lo(in_b)(a4)
	sw	a1,%lo(in_b+4)(a4)
	sw	a2,%lo(in_a)(a5)
	sw	a3,%lo(in_a+4)(a5)
	jalr	zero,ra,0
	.size	initialise_benchmark, .-initialise_benchmark
	.align	2
	.globl	verify_benchmark
	.type	verify_benchmark, @function
verify_benchmark:
	addi	op_0,x0,1
	bgeu	a0,op_0,.+8
	jal	x0,.+12
	addi	a0,x0,0
	jal	x0,.+8
	addi	a0,x0,1
	jalr	zero,ra,0
	.size	verify_benchmark, .-verify_benchmark
	.section	.srodata.cst8,"aM",@progbits,8
	.align	3
.LC0:
	.word	958986399
	.word	-85440217
	.align	3
.LC1:
	.word	-1187838605
	.word	343109103
	.align	3
.LC2:
	.word	-2027716625
	.word	88684321
	.section	.sbss,"aw",@nobits
	.align	3
	.type	in_m, @object
	.size	in_m, 8
in_m:
	.zero	8
	.type	in_b, @object
	.size	in_b, 8
in_b:
	.zero	8
	.type	in_a, @object
	.size	in_a, 8
in_a:
	.zero	8
	.ident	"GCC: (g1b306039a) 15.1.0"
	.section	.note.GNU-stack,"",@progbits


    .text
    .align 2
__mul:
	sub	op_0,x0,a0
	sub	a2,x0,op_0
	addi	a0,x0,0
.Mul_loop:
	addi	op_2,x0,1
	or	op_1,a1,op_2
	addi	op_3,x0,1
	sub	op_0,op_1,op_3
	sub	a3,a1,op_0
	bne	a3,x0,.+8
	jal	x0,.Mul_skip
	sub	op_0,x0,a0
	sub	a0,a2,op_0
.Mul_skip:
	addi	op_0,x0,1
	srl	a1,a1,op_0
	addi	op_0,x0,1
	sll	a2,a2,op_0
	bne	a1,x0,.Mul_loop
	jalr	x0,ra,0

