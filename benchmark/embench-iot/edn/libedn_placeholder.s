	.file	"libedn.c"
	.option nopic
	.option norelax
	.attribute arch, "rv32i2p1_m2p0_zicsr2p0_zifencei2p0_zmmul1p0"
	.attribute unaligned_access, 0
	.attribute stack_align, 16
	.text
	.align	2
	.globl	initialise_board
	.type	initialise_board, @function
initialise_board:
	jalr	zero,ra,0
	.size	initialise_board, .-initialise_board
	.align	2
	.globl	start_trigger
	.type	start_trigger, @function
start_trigger:
	jalr	zero,ra,0
	.size	start_trigger, .-start_trigger
	.align	2
	.globl	stop_trigger
	.type	stop_trigger, @function
stop_trigger:
	jalr	zero,ra,0
	.size	stop_trigger, .-stop_trigger
	.align	2
	.globl	vec_mpy1
	.type	vec_mpy1, @function
vec_mpy1:
	ori	op_0,x0,300
	sub	op_1,x0,a0
	sub	a3,op_0,op_1
.L6:
	lh	a5,0(a1)
	lhu	a4,0(a0)
	ori	op_0,x0,2
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	callmul	a5,a5,a2
	ori	op_0,x0,2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a4,op_0
	sh	a5,-2(a0)
	beq	a3,a0,.+8
	jal	x0,.L6
	jalr	zero,ra,0
	.size	vec_mpy1, .-vec_mpy1
	.align	2
	.globl	mac
	.type	mac, @function
mac:
	lw	a7,0(a3)
	ori	op_0,x0,0
	sub	op_1,x0,a0
	sub	a6,op_0,op_1
	ori	op_0,x0,300
	sub	op_1,x0,a1
	sub	t1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	a0,op_0,op_1
.L9:
	lh	a4,0(a1)
	lh	a5,0(a6)
	ori	op_0,x0,2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a2,a4,a4
	ori	op_0,x0,2
	sub	op_1,x0,a6
	sub	a6,op_0,op_1
	callmul	a5,a5,a4
	sub	op_0,x0,a0
	sub	a0,a2,op_0
	sub	op_0,x0,a7
	sub	a7,a5,op_0
	beq	t1,a1,.+8
	jal	x0,.L9
	sw	a7,0(a3)
	jalr	zero,ra,0
	.size	mac, .-mac
	.align	2
	.globl	fir
	.type	fir, @function
fir:
	ori	op_0,x0,100
	sub	op_1,x0,a0
	sub	a6,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	a7,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	t1,op_0,op_1
	ori	op_0,x0,50
	sub	op_1,x0,zero
	sub	t3,op_0,op_1
.L12:
	ori	op_0,x0,-100
	sub	op_1,x0,a6
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a1
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a2,op_0,op_1
.L13:
	lh	a4,0(a5)
	lh	a0,0(a3)
	ori	op_0,x0,2
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	callmul	a4,a4,a0
	sub	op_0,x0,a2
	sub	a2,a4,op_0
	beq	a6,a5,.+8
	jal	x0,.L13
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a2,a2,op_0
	sw	a2,0(a7)
	ori	op_0,x0,1
	sub	op_1,x0,t1
	sub	t1,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a7
	sub	a7,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a6
	sub	a6,op_0,op_1
	beq	t1,t3,.+8
	jal	x0,.L12
	jalr	zero,ra,0
	.size	fir, .-fir
	.align	2
	.globl	fir_no_red_ld
	.type	fir_no_red_ld, @function
fir_no_red_ld:
	ori	op_0,x0,66
	sub	op_1,x0,a0
	sub	t4,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	t5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	t6,op_0,op_1
	ori	op_0,x0,100
	sub	op_1,x0,zero
	sub	t0,op_0,op_1
.L18:
	lh	t1,-66(t4)
	ori	op_0,x0,-64
	sub	op_1,x0,t4
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a1
	sub	a7,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a6,op_0,op_1
.L17:
	lh	a2,0(a7)
	lh	a3,0(a5)
	lh	a4,2(a7)
	callmul	t3,t1,a2
	lh	t1,2(a5)
	ori	op_0,x0,4
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a7
	sub	a7,op_0,op_1
	callmul	a2,a2,a3
	sub	op_0,x0,t3
	sub	a6,a6,op_0
	callmul	a3,a3,a4
	sub	op_0,x0,a2
	sub	a0,a0,op_0
	callmul	a4,t1,a4
	sub	op_0,x0,a3
	sub	a6,a6,op_0
	sub	op_0,x0,a4
	sub	a0,a0,op_0
	beq	t4,a5,.+8
	jal	x0,.L17
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a6,a6,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a0,a0,op_0
	sw	a6,0(t5)
	sw	a0,4(t5)
	ori	op_0,x0,2
	sub	op_1,x0,t6
	sub	t6,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,t5
	sub	t5,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,t4
	sub	t4,op_0,op_1
	beq	t6,t0,.+8
	jal	x0,.L18
	jalr	zero,ra,0
	.size	fir_no_red_ld, .-fir_no_red_ld
	.align	2
	.globl	latsynth
	.type	latsynth, @function
latsynth:
	slli	a4,a2,1
	ori	op_0,x0,-2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	sub	op_0,x0,a1
	sub	a6,a4,op_0
	sub	op_0,x0,a0
	sub	a5,a4,op_0
	lh	a7,0(a6)
	lh	a5,0(a5)
	ori	op_0,x0,1
	sub	op_1,x0,zero
	sub	a6,op_0,op_1
	callmul	a5,a5,a7
	sub	a3,a3,a5
	bge	a6,a2,.L22
	ori	op_0,x0,2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	sub	op_0,x0,a1
	sub	a1,a4,op_0
	ori	op_0,x0,2
	sub	op_1,x0,a0
	sub	a7,op_0,op_1
	sub	op_0,x0,a0
	sub	a4,a4,op_0
.L23:
	lh	a5,-4(a1)
	lh	a2,-4(a4)
	ori	op_0,x0,-2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,-2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a6,a2,a5
	sub	a3,a3,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a6,a3,op_0
	callmul	a5,a5,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a2,op_0
	sh	a5,0(a4)
	beq	a7,a4,.+8
	jal	x0,.L23
	sh	a6,0(a0)
	ori	op_0,x0,0
	sub	op_1,x0,a3
	sub	a0,op_0,op_1
	jalr	zero,ra,0
.L22:
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a6,a3,op_0
	sh	a6,0(a0)
	ori	op_0,x0,0
	sub	op_1,x0,a3
	sub	a0,op_0,op_1
	jalr	zero,ra,0
	.size	latsynth, .-latsynth
	.align	2
	.globl	iir1
	.type	iir1, @function
iir1:
	lh	a1,0(a1)
	ori	op_0,x0,400
	sub	op_1,x0,a0
	sub	t4,op_0,op_1
.L27:
	lw	a7,0(a3)
	lh	a4,4(a0)
	lw	t3,4(a3)
	lh	t1,6(a0)
	lh	a6,2(a0)
	lh	a5,0(a0)
	callmul	a4,a4,a7
	sw	a7,4(a3)
	ori	op_0,x0,8
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	callmul	t1,t1,t3
	callmul	a6,a6,t3
	sub	op_0,x0,a4
	sub	a4,t1,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,a4,op_0
	sub	op_0,x0,a4
	sub	a4,a1,op_0
	sw	a4,-8(a3)
	callmul	a5,a5,a7
	sub	op_0,x0,a5
	sub	a5,a6,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a1,a4,op_0
	beq	a0,t4,.+8
	jal	x0,.L27
	sw	a1,0(a2)
	jalr	zero,ra,0
	.size	iir1, .-iir1
	.align	2
	.globl	codebook
	.type	codebook, @function
codebook:
	ori	op_0,x0,0
	sub	op_1,x0,a4
	sub	a0,op_0,op_1
	jalr	zero,ra,0
	.size	codebook, .-codebook
	.align	2
	.globl	jpegdct
	.type	jpegdct, @function
jpegdct:
	ori	op_0,x0,-64
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	sw	s5,40(sp)
	sw	s0,60(sp)
	sw	s1,56(sp)
	sw	s2,52(sp)
	sw	s3,48(sp)
	sw	s4,44(sp)
	sw	s6,36(sp)
	sw	s7,32(sp)
	sw	s8,28(sp)
	sw	s9,24(sp)
	sw	s10,20(sp)
	sw	s11,16(sp)
	ori	op_0,x0,0
	sub	op_1,x0,a0
	sub	s5,op_0,op_1
	ori	op_0,x0,14
	sub	op_1,x0,a0
	sub	t6,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a0
	sub	t5,op_0,op_1
	ori	op_0,x0,12
	sub	op_1,x0,a0
	sub	t4,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a0
	sub	t3,op_0,op_1
	ori	op_0,x0,10
	sub	op_1,x0,a0
	sub	t1,op_0,op_1
	ori	op_0,x0,6
	sub	op_1,x0,a0
	sub	a7,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,a0
	sub	a6,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,zero
	sub	a2,op_0,op_1
.L31:
	lh	a3,0(t6)
	lh	s3,0(a6)
	lh	s2,0(t4)
	lh	s1,0(t1)
	lh	t0,0(a0)
	lh	a4,0(a7)
	lh	s0,0(t5)
	lh	a5,0(t3)
	sub	op_0,x0,a4
	sub	s4,s3,op_0
	sub	op_0,x0,s0
	sub	s7,s2,op_0
	sub	op_0,x0,a5
	sub	s6,s1,op_0
	sub	op_0,x0,t0
	sub	t2,a3,op_0
	sub	t0,t0,a3
	sub	op_0,x0,t2
	sub	a3,s4,op_0
	sub	t2,t2,s4
	sub	op_0,x0,s7
	sub	s4,s6,op_0
	sub	s7,s7,s6
	sub	op_0,x0,a3
	sub	s6,s4,op_0
	sh	s6,0(a0)
	sub	a3,a3,s4
	sh	a3,0(a6)
	lh	a3,18(a1)
	lh	s8,20(a1)
	sub	op_0,x0,s7
	sub	s6,t2,op_0
	callmul	a3,a3,t2
	sub	a4,a4,s3
	sub	s0,s0,s2
	sub	a5,a5,s1
	sub	op_0,x0,s0
	sub	s3,a4,op_0
	sub	op_0,x0,t0
	sub	s1,a5,op_0
	sub	op_0,x0,s3
	sub	s4,s1,op_0
	sub	op_0,x0,a4
	sub	t2,t0,op_0
	sub	op_0,x0,a5
	sub	s2,s0,op_0
	ori	op_0,x0,-1
	sub	op_1,x0,a2
	sub	a2,op_0,op_1
	callmul	s6,s6,s8
	ori	op_1,x0,13
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a3,a3,op_0
	slli	a2,a2,16
	srli	a2,a2,16
	ori	op_0,x0,16
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a6
	sub	a6,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t3
	sub	t3,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t4
	sub	t4,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t6
	sub	t6,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t1
	sub	t1,op_0,op_1
	sub	op_0,x0,s6
	sub	a3,a3,op_0
	sh	a3,-16(t3)
	lh	a3,22(a1)
	ori	op_0,x0,16
	sub	op_1,x0,a7
	sub	a7,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t5
	sub	t5,op_0,op_1
	callmul	a3,a3,s7
	ori	op_1,x0,13
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a3,a3,op_0
	sub	op_0,x0,s6
	sub	s6,a3,op_0
	sh	s6,-16(t4)
	lh	s6,16(a1)
	lhu	s7,8(a1)
	lh	a3,2(a1)
	lh	s8,4(a1)
	callmul	s4,s4,s6
	lh	s6,6(a1)
	callmul	a3,a3,s3
	lh	s3,0(a1)
	callmul	t2,t2,s8
	sub	op_0,x0,a3
	sub	a3,s4,op_0
	callmul	a4,a4,s7
	sub	op_0,x0,t2
	sub	a4,a4,op_0
	sub	op_0,x0,a3
	sub	a4,a4,op_0
	slli	s7,a4,16
	ori	op_1,x0,29
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,s7,op_0
	sh	a4,-16(t6)
	lhu	s7,12(a1)
	callmul	a4,s6,s1
	callmul	a5,a5,s7
	sub	op_0,x0,a4
	sub	a4,s4,op_0
	callmul	s1,s2,s3
	sub	op_0,x0,s1
	sub	a5,a5,op_0
	sub	op_0,x0,a4
	sub	a5,a5,op_0
	slli	s2,a5,16
	ori	op_1,x0,29
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,s2,op_0
	sh	a5,-16(t1)
	lhu	a5,10(a1)
	callmul	s0,s0,a5
	sub	op_0,x0,s1
	sub	s1,s0,op_0
	sub	op_0,x0,a3
	sub	a3,s1,op_0
	slli	a5,a3,16
	ori	op_1,x0,29
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a3,a5,op_0
	sh	a3,-16(a7)
	lhu	a5,14(a1)
	callmul	t0,t0,a5
	sub	op_0,x0,t2
	sub	t2,t0,op_0
	sub	op_0,x0,a4
	sub	a4,t2,op_0
	slli	a5,a4,16
	ori	op_1,x0,29
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,a5,op_0
	sh	a4,-16(t5)
	beq	a2,zero,.+8
	jal	x0,.L31
	ori	op_0,x0,16
	sub	op_1,x0,zero
	sub	s7,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	t1,op_0,op_1
	ori	op_0,x0,1
	sub	op_1,x0,zero
	sub	s10,op_0,op_1
	ori	op_0,x0,126
	sub	op_1,x0,zero
	sub	s11,op_0,op_1
	ori	op_0,x0,14
	sub	op_1,x0,zero
	sub	s8,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,zero
	sub	s3,op_0,op_1
.L33:
	beq	s3,s10,.+8
	jal	x0,.L38
	lw	s0,60(sp)
	lw	s1,56(sp)
	lw	s2,52(sp)
	lw	s3,48(sp)
	lw	s4,44(sp)
	lw	s5,40(sp)
	lw	s6,36(sp)
	lw	s7,32(sp)
	lw	s8,28(sp)
	lw	s9,24(sp)
	lw	s10,20(sp)
	lw	s11,16(sp)
	ori	op_0,x0,64
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
.L38:
	sub	op_0,x0,s5
	sub	s5,s7,op_0
	sub	t3,s11,s8
	sub	op_0,x0,t3
	sub	t3,s5,op_0
	ori	op_0,x0,-128
	sub	op_1,x0,t3
	sub	s5,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,zero
	sub	s7,op_0,op_1
	ori	op_0,x0,112
	sub	op_1,x0,zero
	sub	s8,op_0,op_1
	ori	op_0,x0,1
	sub	op_1,x0,zero
	sub	s3,op_0,op_1
	ori	op_0,x0,-16
	sub	op_1,x0,t3
	sub	s1,op_0,op_1
	ori	op_0,x0,-112
	sub	op_1,x0,t3
	sub	s0,op_0,op_1
	ori	op_0,x0,-32
	sub	op_1,x0,t3
	sub	t2,op_0,op_1
	ori	op_0,x0,-96
	sub	op_1,x0,t3
	sub	t0,op_0,op_1
	ori	op_0,x0,-48
	sub	op_1,x0,t3
	sub	t6,op_0,op_1
	ori	op_0,x0,-80
	sub	op_1,x0,t3
	sub	t5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s5
	sub	s2,op_0,op_1
	ori	op_0,x0,-64
	sub	op_1,x0,t3
	sub	t3,op_0,op_1
	ori	op_0,x0,3
	sub	op_1,x0,t1
	sub	s6,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,t1
	sub	a7,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,zero
	sub	t4,op_0,op_1
	sw	s8,4(sp)
	sw	s7,8(sp)
	sw	s3,12(sp)
.L32:
	lh	t1,0(s1)
	lh	a0,0(t3)
	lh	a3,0(s2)
	lh	a4,0(t5)
	lh	a6,0(t2)
	lh	s4,0(t6)
	lh	a2,0(s0)
	lh	a5,0(t0)
	sub	op_0,x0,t1
	sub	s3,a3,op_0
	sub	a3,a3,t1
	sub	op_0,x0,a0
	sub	t1,a4,op_0
	sub	a4,a4,a0
	sub	op_0,x0,t1
	sub	a0,s3,op_0
	sub	s3,s3,t1
	sub	op_0,x0,a6
	sub	t1,a2,op_0
	sub	a2,a2,a6
	sub	op_0,x0,s4
	sub	a6,a5,op_0
	sub	a5,a5,s4
	sub	op_0,x0,a6
	sub	s4,t1,op_0
	sub	t1,t1,a6
	sub	op_0,x0,s4
	sub	a6,a0,op_0
	sra	a6,a6,s6
	sub	a0,a0,s4
	sh	a6,0(s2)
	sra	a0,a0,s6
	sh	a0,0(t3)
	lh	a0,18(a1)
	lh	a6,20(a1)
	sub	op_0,x0,s3
	sub	s7,t1,op_0
	callmul	a0,a0,s3
	sub	op_0,x0,a4
	sub	s4,a2,op_0
	sub	op_0,x0,a5
	sub	s3,a3,op_0
	sub	op_0,x0,s3
	sub	s9,s4,op_0
	sub	op_0,x0,a3
	sub	s8,a4,op_0
	ori	op_0,x0,-1
	sub	op_1,x0,t4
	sub	t4,op_0,op_1
	slli	t4,t4,16
	srli	t4,t4,16
	ori	op_0,x0,2
	sub	op_1,x0,s2
	sub	s2,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,t3
	sub	t3,op_0,op_1
	callmul	a6,a6,s7
	sra	a0,a0,a7
	sub	op_0,x0,a2
	sub	s7,a5,op_0
	ori	op_0,x0,2
	sub	op_1,x0,t0
	sub	t0,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,t2
	sub	t2,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,s1
	sub	s1,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,t6
	sub	t6,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,t5
	sub	t5,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,s0
	sub	s0,op_0,op_1
	sub	op_0,x0,a6
	sub	a0,a0,op_0
	sh	a0,-2(t0)
	lh	a0,22(a1)
	callmul	a0,a0,t1
	sra	a0,a0,a7
	sub	op_0,x0,a6
	sub	a6,a0,op_0
	sh	a6,-2(t2)
	lh	t1,2(a1)
	lh	a0,6(a1)
	lh	a6,16(a1)
	callmul	t1,t1,s4
	lhu	s4,8(a1)
	callmul	a0,a0,s3
	lh	s3,4(a1)
	callmul	a6,a6,s9
	callmul	s3,s3,s8
	sub	op_0,x0,t1
	sub	t1,a6,op_0
	sub	op_0,x0,a0
	sub	a0,a6,op_0
	lh	a6,0(a1)
	callmul	a4,a4,s4
	sub	op_0,x0,s3
	sub	a4,a4,op_0
	sub	op_0,x0,t1
	sub	a4,a4,op_0
	slli	a4,a4,16
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,a4,op_0
	sra	a4,a4,a7
	sh	a4,-2(s1)
	lhu	a4,12(a1)
	callmul	a6,a6,s7
	callmul	a5,a5,a4
	sub	op_0,x0,a6
	sub	a5,a5,op_0
	sub	op_0,x0,a0
	sub	a5,a5,op_0
	slli	a5,a5,16
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sra	a5,a5,a7
	sh	a5,-2(t6)
	lhu	a5,10(a1)
	callmul	a2,a2,a5
	sub	op_0,x0,a6
	sub	a6,a2,op_0
	sub	op_0,x0,t1
	sub	t1,a6,op_0
	slli	t1,t1,16
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	t1,t1,op_0
	sra	t1,t1,a7
	sh	t1,-2(t5)
	lhu	a5,14(a1)
	callmul	a3,a3,a5
	sub	op_0,x0,s3
	sub	s3,a3,op_0
	sub	op_0,x0,a0
	sub	a0,s3,op_0
	slli	a0,a0,16
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a0,a0,op_0
	sra	a0,a0,a7
	sh	a0,-2(s0)
	beq	t4,zero,.+8
	jal	x0,.L32
	lw	s8,4(sp)
	lw	s7,8(sp)
	lw	s3,12(sp)
	ori	op_0,x0,0
	sub	op_1,x0,s6
	sub	t1,op_0,op_1
	jal	x0,.L33
	.size	jpegdct, .-jpegdct
	.align	2
	.type	benchmark_body.constprop.0.isra.0, @function
benchmark_body.constprop.0.isra.0:
	ori	op_0,x0,-864
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	lui	a2,%hi(.LANCHOR0)
	sw	s0,856(sp)
	sw	s1,852(sp)
	sw	s2,848(sp)
	sw	s3,844(sp)
	sw	s9,820(sp)
	sw	s10,816(sp)
	sw	s11,812(sp)
	lui	s2,%hi(.LANCHOR1)
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a2
	sub	a2,op_0,op_1
	lui	s1,%hi(.LANCHOR1+400)
	lui	s9,%hi(.LANCHOR0+1200)
	lui	a0,%hi(.LANCHOR0+800)
	lui	s11,%hi(.LANCHOR0+1100)
	lui	s10,%hi(.LANCHOR0+1500)
	lui	s0,%hi(.LANCHOR0+1300)
	lui	t5,%hi(.LANCHOR0+798)
	lui	s3,11
	sw	s4,840(sp)
	sw	s5,836(sp)
	sw	s6,832(sp)
	sw	s7,828(sp)
	sw	s8,824(sp)
	sw	ra,860(sp)
	ori	op_0,x0,%lo(.LANCHOR1)
	sub	op_1,x0,s2
	sub	s2,op_0,op_1
	ori	op_0,x0,1600
	sub	op_1,x0,a2
	sub	s8,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR1+400)
	sub	op_1,x0,s1
	sub	s1,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1200)
	sub	op_1,x0,s9
	sub	s9,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+800)
	sub	op_1,x0,a0
	sub	s7,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1100)
	sub	op_1,x0,s11
	sub	s11,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1500)
	sub	op_1,x0,s10
	sub	s10,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1300)
	sub	op_1,x0,s0
	sub	s0,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+798)
	sub	op_1,x0,t5
	sub	s6,op_0,op_1
	ori	op_0,x0,-1366
	sub	op_1,x0,s3
	sub	s3,op_0,op_1
	ori	op_0,x0,87
	sub	op_1,x0,zero
	sub	s5,op_0,op_1
	ori	op_0,x0,50
	sub	op_1,x0,zero
	sub	s4,op_0,op_1
.L50:
	ori	op_0,x0,0
	sub	op_1,x0,s2
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L40:
	lw	a6,0(a5)
	lw	a0,4(a5)
	lw	a1,8(a5)
	lw	a3,12(a5)
	sw	a6,0(a4)
	sw	a0,4(a4)
	sw	a1,8(a4)
	sw	a3,12(a4)
	ori	op_0,x0,16
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a5,s1,.+8
	jal	x0,.L40
	ori	op_0,x0,400
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L41:
	lw	a3,12(a5)
	lw	a6,0(a5)
	lw	a0,4(a5)
	lw	a1,8(a5)
	sw	a3,12(a4)
	lui	a3,%hi(.LANCHOR1+800)
	sw	a6,0(a4)
	sw	a0,4(a4)
	sw	a1,8(a4)
	ori	op_0,x0,16
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR1+800)
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a5,a3,.+8
	jal	x0,.L41
	ori	op_0,x0,400
	sub	op_1,x0,sp
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,sp
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a4,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a5,op_0,op_1
.L42:
	lw	a6,0(a3)
	lw	a0,0(a1)
	ori	op_0,x0,4
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	sw	a6,0(a4)
	sw	a0,-4(a5)
	ori	op_0,x0,4
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	beq	s8,a5,.+8
	jal	x0,.L42
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a4,op_0,op_1
.L43:
	lh	a1,0(a3)
	lhu	a0,0(a4)
	ori	op_0,x0,2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	slli	a5,a1,1
	sub	op_0,x0,a5
	sub	a5,a1,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a0,op_0
	sh	a5,-2(a4)
	ori	op_0,x0,2
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	beq	s11,a4,.+8
	jal	x0,.L43
	lw	a6,0(a2)
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a5,op_0,op_1
	ori	op_0,x0,3
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
.L44:
	lh	a3,0(a5)
	lh	a4,0(a1)
	ori	op_0,x0,2
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	callmul	a7,a3,a3
	ori	op_0,x0,2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a4,a4,a3
	sub	op_0,x0,a0
	sub	a0,a7,op_0
	sub	op_0,x0,a6
	sub	a6,a4,op_0
	beq	s10,a5,.+8
	jal	x0,.L44
	lui	a5,%hi(c)
	sw	a6,0(a2)
	sh	a0,%lo(c)(a5)
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	t1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a7,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a6,op_0,op_1
.L45:
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a7
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a1,op_0,op_1
.L46:
	lh	a4,0(a3)
	lh	a0,0(a5)
	ori	op_0,x0,2
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	callmul	a4,a4,a0
	sub	op_0,x0,a1
	sub	a1,a4,op_0
	beq	s0,a5,.+8
	jal	x0,.L46
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a1,a1,op_0
	sw	a1,0(t1)
	ori	op_0,x0,1
	sub	op_1,x0,a6
	sub	a6,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,t1
	sub	t1,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a7
	sub	a7,op_0,op_1
	beq	a6,s4,.+8
	jal	x0,.L45
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a0,op_0,op_1
.Lpcrel_1:
	auipc	ra,%pcrel_hi(fir_no_red_ld)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_1)
	lui	a5,%hi(.LANCHOR0)
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a5
	sub	a2,op_0,op_1
	lh	a3,998(a2)
	lh	a5,1398(a2)
	lui	a4,%hi(.LANCHOR0+996)
	lui	a1,%hi(.LANCHOR0+1396)
	callmul	a3,a3,a5
	ori	op_0,x0,%lo(.LANCHOR0+996)
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1396)
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	sub	a3,s3,a3
.L48:
	lh	a0,0(a4)
	lh	a5,0(a1)
	ori	op_0,x0,-2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,-2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a6,a0,a5
	sub	a3,a3,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a6,a3,op_0
	callmul	a5,a5,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a0,op_0
	sh	a5,4(a4)
	beq	s6,a4,.+8
	jal	x0,.L48
	lh	t1,1200(a2)
	lui	a4,%hi(d)
	sh	a6,800(a2)
	sw	a3,%lo(d)(a4)
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	a0,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a1,op_0,op_1
.L49:
	lw	a6,0(a0)
	lh	a5,4(a1)
	lh	a4,0(a1)
	lw	t3,4(a0)
	lh	a7,6(a1)
	callmul	a5,a5,a6
	sw	a6,4(a0)
	ori	op_0,x0,8
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	callmul	a4,a4,a6
	lh	a6,-6(a1)
	callmul	a7,a7,t3
	callmul	a6,a6,t3
	sub	op_0,x0,a5
	sub	a5,a7,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,t1,op_0
	sw	a5,-8(a0)
	sub	op_0,x0,a4
	sub	a4,a6,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,a4,op_0
	sub	op_0,x0,a4
	sub	t1,a5,op_0
	beq	a1,s9,.+8
	jal	x0,.L49
	lui	a5,%hi(e)
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a0,op_0,op_1
	sw	t1,400(a2)
	sw	a3,%lo(e)(a5)
.Lpcrel_2:
	auipc	ra,%pcrel_hi(jpegdct)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_2)
	lui	a5,%hi(.LANCHOR0)
	ori	op_0,x0,-1
	sub	op_1,x0,s5
	sub	s5,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a5
	sub	a2,op_0,op_1
	beq	s5,zero,.+8
	jal	x0,.L50
	lw	ra,860(sp)
	lw	s0,856(sp)
	lw	s1,852(sp)
	lw	s2,848(sp)
	lw	s3,844(sp)
	lw	s4,840(sp)
	lw	s5,836(sp)
	lw	s6,832(sp)
	lw	s7,828(sp)
	lw	s8,824(sp)
	lw	s9,820(sp)
	lw	s10,816(sp)
	lw	s11,812(sp)
	ori	op_0,x0,864
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
	.size	benchmark_body.constprop.0.isra.0, .-benchmark_body.constprop.0.isra.0
	.align	2
	.type	benchmark_body.isra.0, @function
benchmark_body.isra.0:
	bge	zero,a0,.L84
	ori	op_0,x0,-880
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	lui	a2,%hi(.LANCHOR0)
	sw	s0,872(sp)
	sw	s1,868(sp)
	sw	s2,864(sp)
	sw	s4,856(sp)
	sw	s9,836(sp)
	sw	s10,832(sp)
	sw	s11,828(sp)
	sw	a0,12(sp)
	lui	s2,%hi(.LANCHOR1)
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a2
	sub	a2,op_0,op_1
	lui	s1,%hi(.LANCHOR1+400)
	lui	s9,%hi(.LANCHOR0+1200)
	lui	a0,%hi(.LANCHOR0+800)
	lui	s11,%hi(.LANCHOR0+1100)
	lui	s10,%hi(.LANCHOR0+1500)
	lui	s0,%hi(.LANCHOR0+1300)
	lui	t6,%hi(.LANCHOR0+798)
	lui	s4,11
	sw	s3,860(sp)
	sw	s5,852(sp)
	sw	s6,848(sp)
	sw	s7,844(sp)
	sw	s8,840(sp)
	sw	ra,876(sp)
	ori	op_0,x0,%lo(.LANCHOR1)
	sub	op_1,x0,s2
	sub	s2,op_0,op_1
	ori	op_0,x0,1600
	sub	op_1,x0,a2
	sub	s8,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR1+400)
	sub	op_1,x0,s1
	sub	s1,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1200)
	sub	op_1,x0,s9
	sub	s9,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+800)
	sub	op_1,x0,a0
	sub	s7,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1100)
	sub	op_1,x0,s11
	sub	s11,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1500)
	sub	op_1,x0,s10
	sub	s10,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1300)
	sub	op_1,x0,s0
	sub	s0,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+798)
	sub	op_1,x0,t6
	sub	s6,op_0,op_1
	ori	op_0,x0,-1366
	sub	op_1,x0,s4
	sub	s4,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	s5,op_0,op_1
	ori	op_0,x0,50
	sub	op_1,x0,zero
	sub	s3,op_0,op_1
.L73:
	ori	op_0,x0,0
	sub	op_1,x0,s2
	sub	a5,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L63:
	lw	a6,0(a5)
	lw	a0,4(a5)
	lw	a1,8(a5)
	lw	a3,12(a5)
	sw	a6,0(a4)
	sw	a0,4(a4)
	sw	a1,8(a4)
	sw	a3,12(a4)
	ori	op_0,x0,16
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a5,s1,.+8
	jal	x0,.L63
	ori	op_0,x0,416
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L64:
	lw	a3,12(a5)
	lw	a6,0(a5)
	lw	a0,4(a5)
	lw	a1,8(a5)
	sw	a3,12(a4)
	lui	a3,%hi(.LANCHOR1+800)
	sw	a6,0(a4)
	sw	a0,4(a4)
	sw	a1,8(a4)
	ori	op_0,x0,16
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR1+800)
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a5,a3,.+8
	jal	x0,.L64
	ori	op_0,x0,416
	sub	op_1,x0,sp
	sub	a1,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,sp
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a4,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a5,op_0,op_1
.L65:
	lw	a6,0(a3)
	lw	a0,0(a1)
	ori	op_0,x0,4
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	sw	a6,0(a4)
	sw	a0,-4(a5)
	ori	op_0,x0,4
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	beq	s8,a5,.+8
	jal	x0,.L65
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a4,op_0,op_1
.L66:
	lh	a1,0(a3)
	lhu	a0,0(a4)
	ori	op_0,x0,2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	slli	a5,a1,1
	sub	op_0,x0,a5
	sub	a5,a1,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a0,op_0
	sh	a5,-2(a4)
	ori	op_0,x0,2
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	beq	s11,a4,.+8
	jal	x0,.L66
	lw	a6,0(a2)
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a4,op_0,op_1
	ori	op_0,x0,3
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
.L67:
	lh	a5,0(a4)
	lh	a3,0(a1)
	ori	op_0,x0,2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a3,a3,a5
	callmul	a5,a5,a5
	sub	op_0,x0,a6
	sub	a6,a3,op_0
	sub	op_0,x0,a0
	sub	a0,a5,op_0
	beq	s10,a4,.+8
	jal	x0,.L67
	lui	a5,%hi(c)
	sw	a6,0(a2)
	sh	a0,%lo(c)(a5)
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	a7,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a6,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
.L68:
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a6
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a1,op_0,op_1
.L69:
	lh	a4,0(a3)
	lh	t1,0(a5)
	ori	op_0,x0,2
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a3
	sub	a3,op_0,op_1
	callmul	a4,a4,t1
	sub	op_0,x0,a1
	sub	a1,a4,op_0
	beq	s0,a5,.+8
	jal	x0,.L69
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a1,a1,op_0
	sw	a1,0(a7)
	ori	op_0,x0,1
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	ori	op_0,x0,4
	sub	op_1,x0,a7
	sub	a7,op_0,op_1
	ori	op_0,x0,2
	sub	op_1,x0,a6
	sub	a6,op_0,op_1
	beq	a0,s3,.+8
	jal	x0,.L68
	ori	op_0,x0,0
	sub	op_1,x0,s9
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a0,op_0,op_1
.Lpcrel_3:
	auipc	ra,%pcrel_hi(fir_no_red_ld)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_3)
	lui	a5,%hi(.LANCHOR0)
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a5
	sub	a2,op_0,op_1
	lh	a3,998(a2)
	lh	a5,1398(a2)
	lui	a4,%hi(.LANCHOR0+996)
	lui	a1,%hi(.LANCHOR0+1396)
	callmul	a3,a3,a5
	ori	op_0,x0,%lo(.LANCHOR0+996)
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0+1396)
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	sub	a3,s4,a3
.L71:
	lh	a0,0(a4)
	lh	a5,0(a1)
	ori	op_0,x0,-2
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,-2
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	callmul	a6,a0,a5
	sub	a3,a3,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a6,a3,op_0
	callmul	a5,a5,a6
	ori	op_1,x0,16
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,a0,op_0
	sh	a5,4(a4)
	beq	s6,a4,.+8
	jal	x0,.L71
	lh	t1,1200(a2)
	lui	a4,%hi(d)
	sh	a6,800(a2)
	sw	a3,%lo(d)(a4)
	ori	op_0,x0,0
	sub	op_1,x0,a2
	sub	a0,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a1,op_0,op_1
.L72:
	lw	a6,0(a0)
	lh	a5,4(a1)
	lh	a4,0(a1)
	lw	t3,4(a0)
	lh	a7,6(a1)
	callmul	a5,a5,a6
	sw	a6,4(a0)
	ori	op_0,x0,8
	sub	op_1,x0,a1
	sub	a1,op_0,op_1
	ori	op_0,x0,8
	sub	op_1,x0,a0
	sub	a0,op_0,op_1
	callmul	a4,a4,a6
	lh	a6,-6(a1)
	callmul	a7,a7,t3
	callmul	a6,a6,t3
	sub	op_0,x0,a5
	sub	a5,a7,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a5,a5,op_0
	sub	op_0,x0,a5
	sub	a5,t1,op_0
	sw	a5,-8(a0)
	sub	op_0,x0,a4
	sub	a4,a6,op_0
	ori	op_1,x0,15
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	sra	a4,a4,op_0
	sub	op_0,x0,a4
	sub	t1,a5,op_0
	beq	a1,s9,.+8
	jal	x0,.L72
	lui	a5,%hi(e)
	ori	op_0,x0,0
	sub	op_1,x0,s7
	sub	a0,op_0,op_1
	sw	t1,400(a2)
	sw	a3,%lo(e)(a5)
.Lpcrel_4:
	auipc	ra,%pcrel_hi(jpegdct)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_4)
	lw	a5,12(sp)
	lui	a4,%hi(.LANCHOR0)
	ori	op_0,x0,1
	sub	op_1,x0,s5
	sub	s5,op_0,op_1
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a4
	sub	a2,op_0,op_1
	beq	s5,a5,.+8
	jal	x0,.L73
	lw	ra,876(sp)
	lw	s0,872(sp)
	lw	s1,868(sp)
	lw	s2,864(sp)
	lw	s3,860(sp)
	lw	s4,856(sp)
	lw	s5,852(sp)
	lw	s6,848(sp)
	lw	s7,844(sp)
	lw	s8,840(sp)
	lw	s9,836(sp)
	lw	s10,832(sp)
	lw	s11,828(sp)
	ori	op_0,x0,880
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
.L84:
	jalr	zero,ra,0
	.size	benchmark_body.isra.0, .-benchmark_body.isra.0
	.align	2
	.globl	initialise_benchmark
	.type	initialise_benchmark, @function
initialise_benchmark:
	jalr	zero,ra,0
	.size	initialise_benchmark, .-initialise_benchmark
	.align	2
	.globl	warm_caches
	.type	warm_caches, @function
warm_caches:
	ori	op_0,x0,-16
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	sw	ra,12(sp)
.Lpcrel_5:
	auipc	ra,%pcrel_hi(benchmark_body.isra.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_5)
	lw	ra,12(sp)
	ori	op_0,x0,16
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
	.size	warm_caches, .-warm_caches
	.align	2
	.globl	benchmark
	.type	benchmark, @function
benchmark:
	ori	op_0,x0,-16
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	sw	ra,12(sp)
.Lpcrel_6:
	auipc	ra,%pcrel_hi(benchmark_body.constprop.0.isra.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_6)
	lw	ra,12(sp)
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
	.size	benchmark, .-benchmark
	.align	2
	.globl	verify_benchmark
	.type	verify_benchmark, @function
verify_benchmark:
	lui	a4,%hi(.LANCHOR1)
	ori	op_0,x0,%lo(.LANCHOR1)
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	ori	op_0,x0,-800
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	ori	op_0,x0,800
	sub	op_1,x0,a4
	sub	a5,op_0,op_1
	ori	op_0,x0,1600
	sub	op_1,x0,a4
	sub	a3,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L93:
	lw	a6,0(a5)
	lw	a0,4(a5)
	lw	a1,8(a5)
	lw	a2,12(a5)
	sw	a6,0(a4)
	sw	a0,4(a4)
	sw	a1,8(a4)
	sw	a2,12(a4)
	ori	op_0,x0,16
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,16
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a5,a3,.+8
	jal	x0,.L93
	lui	a5,%hi(.LANCHOR0)
	ori	op_0,x0,%lo(.LANCHOR0)
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,800
	sub	op_1,x0,a5
	sub	a1,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,sp
	sub	a4,op_0,op_1
.L97:
	lbu	a2,0(a5)
	lbu	a3,0(a4)
	ori	op_0,x0,1
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,1
	sub	op_1,x0,a4
	sub	a4,op_0,op_1
	beq	a2,a3,.+8
	jal	x0,.L103
	beq	a5,a1,.+8
	jal	x0,.L97
	lui	a5,%hi(c)
	lh	a4,%lo(c)(a5)
	lui	a5,3
	ori	op_0,x0,-2045
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	beq	a4,a5,.L104
.L92:
	ori	op_0,x0,800
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
.L103:
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	ori	op_0,x0,800
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
.L104:
	lui	a5,%hi(d)
	lw	a4,%lo(d)(a5)
	lui	a5,940694
	ori	op_0,x0,-1558
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	beq	a4,a5,.+8
	jal	x0,.L92
	lui	a5,%hi(e)
	lw	a0,%lo(e)(a5)
	lui	a5,107882
	ori	op_0,x0,1558
	sub	op_1,x0,a5
	sub	a5,op_0,op_1
	sub	op_0,x0,a0
	sub	a0,a5,op_0
	ori	op_1,x0,1
	sub	op_2,x0,x0
	sub	op_0,op_1,op_2
	bltu	a0,op_0,.+12
	ori	op_3,x0,0
	sub	op_4,x0,x0
	sub	a0,op_3,op_4
	jal	x0,.+8
	ori	op_5,x0,1
	sub	op_6,x0,x0
	sub	a0,op_5,op_6
	jal	x0,.L92
	.size	verify_benchmark, .-verify_benchmark
	.section	.text.startup,"ax",@progbits
	.align	2
	.globl	main
	.type	main, @function
main:
	ori	op_0,x0,-32
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	ori	op_0,x0,1
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	sw	ra,28(sp)
.Lpcrel_7:
	auipc	ra,%pcrel_hi(benchmark_body.isra.0)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_7)
.Lpcrel_8:
	auipc	ra,%pcrel_hi(benchmark)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_8)
	sw	zero,12(sp)
	lw	a0,12(sp)
.Lpcrel_9:
	auipc	ra,%pcrel_hi(verify_benchmark)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_9)
	lw	ra,28(sp)
	ori	op_2,x0,1
	sub	op_3,x0,x0
	sub	op_1,op_2,op_3
	or	op_0,op_1,a0
	ori	op_6,x0,1
	sub	op_7,x0,x0
	sub	op_5,op_6,op_7
	ori	op_11,x0,1
	sub	op_12,x0,x0
	sub	op_10,op_11,op_12
	or	op_9,op_10,a0
	sub	op_8,op_9,a0
	sub	op_4,op_5,op_8
	sub	a0,op_0,op_4
	ori	op_0,x0,32
	sub	op_1,x0,sp
	sub	sp,op_0,op_1
	jalr	zero,ra,0
	.size	main, .-main
	.section	.rodata
	.align	2
	.set	.LANCHOR1,. + 0
.LC0:
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
	.half	0
	.half	2047
	.half	3072
	.half	2048
	.half	512
	.half	-2048
	.half	-3328
	.half	1024
.LC1:
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
	.half	3168
	.half	3136
	.half	3104
	.half	3072
	.half	-2560
	.half	-3072
	.half	-3584
	.half	-4096
.LC2:
	.word	3760
	.word	4269
	.word	3126
	.word	1030
	.word	2453
	.word	-4601
	.word	1981
	.word	-1056
	.word	2621
	.word	4269
	.word	3058
	.word	1030
	.word	2378
	.word	-4601
	.word	1902
	.word	-1056
	.word	2548
	.word	4269
	.word	2988
	.word	1030
	.word	2300
	.word	-4601
	.word	1822
	.word	-1056
	.word	2474
	.word	4269
	.word	2917
	.word	1030
	.word	2220
	.word	-4601
	.word	1738
	.word	-1056
	.word	2398
	.word	4269
	.word	2844
	.word	1030
	.word	2140
	.word	-4601
	.word	1655
	.word	-1056
	.word	2321
	.word	4269
	.word	2770
	.word	1030
	.word	2058
	.word	-4601
	.word	1569
	.word	-1056
	.word	2242
	.word	4269
	.word	2152
	.word	1030
	.word	1683
	.word	-4601
	.word	1627
	.word	-1056
	.word	2030
	.word	4269
	.word	2080
	.word	1030
	.word	1611
	.word	-4601
	.word	1555
	.word	-1056
	.word	1958
	.word	4269
	.word	2008
	.word	1030
	.word	1539
	.word	-4601
	.word	1483
	.word	-1056
	.word	1886
	.word	4269
	.word	1935
	.word	1030
	.word	1466
	.word	-4601
	.word	1410
	.word	-1056
	.word	1813
	.word	4269
	.word	1862
	.word	1030
	.word	1393
	.word	-4601
	.word	1337
	.word	-1056
	.word	1740
	.word	4269
	.word	1789
	.word	1030
	.word	1320
	.word	-4601
	.word	1264
	.word	-1056
	.word	1667
	.word	4269
	.word	1716
	.word	1030
	.word	1968
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.word	0
	.bss
	.align	2
	.set	.LANCHOR0,. + 0
	.type	output, @object
	.size	output, 800
output:
	.zero	800
	.type	a, @object
	.size	a, 400
a:
	.zero	400
	.type	b, @object
	.size	b, 400
b:
	.zero	400
	.section	.sbss,"aw",@nobits
	.align	2
	.type	e, @object
	.size	e, 4
e:
	.zero	4
	.type	d, @object
	.size	d, 4
d:
	.zero	4
	.type	c, @object
	.size	c, 2
c:
	.zero	2
	.ident	"GCC: (g1b306039a) 15.1.0"
	.section	.note.GNU-stack,"",@progbits


    .text
    .align 2
__mul:
	sub	op_0,x0,a0
	sub	a2,x0,op_0
	ori	op_0,x0,0
	sub	op_1,x0,x0
	sub	a0,op_0,op_1
.Mul_loop:
	ori	op_3,x0,1
	sub	op_4,x0,x0
	sub	op_2,op_3,op_4
	or	op_1,a1,op_2
	ori	op_6,x0,1
	sub	op_7,x0,x0
	sub	op_5,op_6,op_7
	sub	op_0,op_1,op_5
	sub	a3,a1,op_0
	beq	a3,x0,.Mul_skip
	sub	op_0,x0,a0
	sub	a0,a2,op_0
.Mul_skip:
	srli	a1,a1,1
	slli	a2,a2,1
	beq	a1,x0,.+8
	jal	x0,.Mul_loop
	jalr	x0,ra,0

.text
.align 2

# Signed 32-bit division: a0 = a0 / a1
.global __riscv_div_lib_divsi3
__riscv_div_lib_divsi3:
	bge	a0,zero,.+8
	jal	x0,__riscv_div_lib_L10
	bge	a1,zero,.+8
	jal	x0,__riscv_div_lib_L11
    # Since the quotient is positive, fall into udivsi3

# Unsigned 32-bit division: a0 = a0 / a1
.global __riscv_div_lib_udivsi3
__riscv_div_lib_udivsi3:
	ori	op_0,x0,0
	sub	op_1,x0,a1
	sub	a2,op_0,op_1
	ori	op_0,x0,0
	sub	op_1,x0,a0
	sub	a1,op_0,op_1
	ori	op_0,x0,-1
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
	beq	a2,zero,__riscv_div_lib_L5
	ori	op_0,x0,1
	sub	op_1,x0,zero
	sub	a3,op_0,op_1
	bltu	a2,a1,.+8
	jal	x0,__riscv_div_lib_L2
__riscv_div_lib_L1:
	bge	zero,a2,__riscv_div_lib_L2
	slli	a2,a2,1
	slli	a3,a3,1
	bltu	a2,a1,__riscv_div_lib_L1
__riscv_div_lib_L2:
	ori	op_0,x0,0
	sub	op_1,x0,zero
	sub	a0,op_0,op_1
__riscv_div_lib_L3:
	bltu	a1,a2,__riscv_div_lib_L4
	sub	a1,a1,a2
	or	a0,a0,a3
__riscv_div_lib_L4:
	srli	a3,a3,1
	srli	a2,a2,1
	beq	a3,zero,.+8
	jal	x0,__riscv_div_lib_L3
__riscv_div_lib_L5:
	jalr	zero,ra,0

# Unsigned 32-bit remainder: a0 = a0 % a1
.global __riscv_div_lib_umodsi3
__riscv_div_lib_umodsi3:
	ori	op_0,x0,0
	sub	op_1,x0,ra
	sub	t0,op_0,op_1
.Lpcrel_div1:
	auipc	ra,%pcrel_hi(__riscv_div_lib_udivsi3)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_div1)
	ori	op_0,x0,0
	sub	op_1,x0,a1
	sub	a0,op_0,op_1
	jalr	zero,t0,0

# Handle negative arguments to divsi3
__riscv_div_lib_L10:
	sub	a0,zero,a0
	bge	zero,a1,.+8
	jal	x0,__riscv_div_lib_L12
	sub	a1,zero,a1
	jal	x0,__riscv_div_lib_udivsi3
__riscv_div_lib_L11:                         # Compute udivsi3(a0, -a1), then negate
    sub   a1, zero, a1                        # neg a1, a1 -> sub a1, zero, a1
__riscv_div_lib_L12:
	ori	op_0,x0,0
	sub	op_1,x0,ra
	sub	t0,op_0,op_1
.Lpcrel_div2:
	auipc	ra,%pcrel_hi(__riscv_div_lib_udivsi3)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_div2)
	sub	a0,zero,a0
	jalr	zero,t0,0

# Signed 32-bit remainder: a0 = a0 % a1
.global __riscv_div_lib_modsi3
__riscv_div_lib_modsi3:
	ori	op_0,x0,0
	sub	op_1,x0,ra
	sub	t0,op_0,op_1
	bge	a1,zero,.+8
	jal	x0,__riscv_div_lib_L31
	bge	a0,zero,.+8
	jal	x0,__riscv_div_lib_L32
__riscv_div_lib_L30:
.Lpcrel_div3:
	auipc	ra,%pcrel_hi(__riscv_div_lib_udivsi3)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_div3)
	ori	op_0,x0,0
	sub	op_1,x0,a1
	sub	a0,op_0,op_1
	jalr	zero,t0,0
__riscv_div_lib_L31:
	sub	a1,zero,a1
	bge	a0,zero,__riscv_div_lib_L30
__riscv_div_lib_L32:
	sub	a0,zero,a0
.Lpcrel_div4:
	auipc	ra,%pcrel_hi(__riscv_div_lib_udivsi3)
	jalr	ra,ra,%pcrel_lo(.Lpcrel_div4)
	sub	a0,zero,a1
	jalr	zero,t0,0

# end of subrountine